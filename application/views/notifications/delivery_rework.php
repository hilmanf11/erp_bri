...
<script>
    ...
    function printDnrNo(dnrNo) {
        window.open('<?= base_url('control/Delivery_rework/print_dn_rework/') ?>' + btoa(dnrNo), '_blank');
    }

    $(function() {
        //SETTING DATAGRID EASYUI
        $('#dg').datagrid({
            url: '<?= base_url('notifications/notification_data/delivery_rework/') ?>' + "<?= base64_encode($user) ?>" + "/" + "<?= base64_encode($name) ?>",
            pagination: false,
            singleSelect: false,
            clientPaging: false,
            rownumbers: true,
            ...
            columns: [
                [{
                    ...
                    formatter: function(value, row) {
                        var print = "printDnrNo('" + row.dnr_no + "')";
