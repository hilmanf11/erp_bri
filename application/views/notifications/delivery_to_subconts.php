...
<script>
    ...
    function printDeliveryNote(deliveryNoteNo) {
        window.open('<?= base_url('control/Delivery_to_subconts/print_dn_to_sc/') ?>' + btoa(deliveryNoteNo), '_blank');
    }

    $(function() {
        //SETTING DATAGRID EASYUI
        $('#dg').datagrid({
            url: '<?= base_url('notifications/notification_data/delivery_to_subconts/') ?>' + "<?= base64_encode($user) ?>" + "/" + "<?= base64_encode($name) ?>",
            pagination: false,
            singleSelect: false,
            clientPaging: false,
            rownumbers: true,
            ...
            columns: [
                [{
                    ...
                    formatter: function(value, row) {
                        var print = "printDeliveryNote('" + row.delivery_note_no + "')";
