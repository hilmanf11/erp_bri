...
<script>
    ...
    function printDeliveryNote(deliveryNoteNo) {
        window.open('<?= base_url('sales/Delivery_notes/print_do/') ?>' + btoa(deliveryNoteNo), '_blank');
    }

    $(function() {
        //SETTING DATAGRID EASYUI
        $('#dg').datagrid({
            url: '<?= base_url('notifications/notification_data/delivery_notes_2/') ?>' + "<?= base64_encode($user) ?>" + "/" + "<?= base64_encode($name) ?>",
            pagination: false,
            singleSelect: false,
            clientPaging: false,
            rownumbers: true,
