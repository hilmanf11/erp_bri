...
<div id="dlg_insert" class="easyui-dialog" title="Add New" data-options="closed: true,modal:true" style="width: 400px; padding:10px; top: 20px;">
    <form id="frm_insert" method="post" novalidate>
        <fieldset style="width:100%; border:1px solid #d0d0d0; margin-bottom: 10px; border-radius:4px; float: left;">
            <legend><b>Add New Lines</b></legend>
            <div class="fitem">
...
<!-- PDF -->
<iframe id="printout" src="<?= base_url('master/line_productions/print') ?>" style="width: 100%;" hidden></iframe>
<script>
    //ADD DATA
    function add() {
    ...
    function update() {
        ...
        if (row) {
            ...
            url_save = '<?= base_url('master/line_productions/update') ?>?id=' + btoa(row.id);
        } else {
            ...
            $.messager.confirm('Warning', 'Are you sure you want to delete this data?', function(r) {
                if (r) {
                    for (var i = 0; i < rows.length; i++) {
                        ...
                        $.ajax({
                            method: 'post',
                            url: '<?= base_url('master/line_productions/delete') ?>',
                            data: {
                                id: row.id
                            },
    ...
    //PRINT EXCEL
    function excel() {
        window.location.assign('<?= base_url('master/line_productions/print/excel') ?>');
    }
    //RELOAD
    function reload() {
