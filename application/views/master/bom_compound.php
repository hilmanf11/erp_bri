...
<!-- PDF -->
<iframe id="printout" src="<?= base_url('master/bom_compound/print') ?>" style="width: 100%;" hidden></iframe>
...
<script>
    ...
    function add() {
        ...
        url_save = '<?= base_url('master/bom_compound/create') ?>';
            ...
            columns: [
                [{
                        ...
                        options: {
                            ...
                            onSelect: function(value, rows) {
                                ...
                                $.when(
                                    $.ajax({
                                        type: "post",
                                        url: "<?= base_url('master/bom_compound/readWeight'); ?>",
                                    ...
                                    $.ajax({
                                        type: "post",
                                        url: "<?= base_url('master/bom_compound/readRunner'); ?>",
                                        data: "item_fg_id=" + item_fg_id,
                                        dataType: "json",
                    ...
                    editor: {
                        type: 'combobox',
                        options: {
                            url: '<?= base_url('master/bom_compound/readUoM'); ?>',
                            required: true,
                            valueField: 'name',
                            textField: 'name',
                            prompt: 'Choose UoM'
    ...
    function removeit() {
        ...
        $.ajax({
            method: 'post',
            url: '<?= base_url('master/bom_compound/delete') ?>',
            data: {
    ...
    function update() {
        ...
        if (row) {
            ...
            addTable('<?= base_url('master/bom_compound/datatableUpdates?item_fg_id=') ?>' + window.btoa(row.item_fg_id));
        } else {
    ...
    function download_excel() {
        window.location.assign('<?= base_url('master/bom_compound/exportTemplate') ?>');
    }
    ...
    function filter() {
        ...
        $('#dg').datagrid({
            url: '<?= base_url('master/bom_compound/datatables') ?>' + url
        });

        $("#printout").contents().find('html').html("<center><br><br><br><b style='font-size:20px;'>Please Wait...</b></center>");
        $("#printout").attr('src', '<?= base_url('master/bom_compound/print') ?>' + url);
    }
    ...
    function excel() {
        ...
        window.location.assign('<?= base_url('master/bom_compound/print/excel') ?>' + url);
    }
    ...
    $(function() {
        ...
        $('#dg').datagrid({
            url: '<?= base_url('master/bom_compound/datatables') ?>',
            pagination: true,
            rownumbers: true,
            fit: true,
            ...
            onExpandRow: function(index, row) {
                ...
                ddv.datagrid({
                    url: '<?= base_url('master/bom_compound/datatableDetails?number=') ?>' + window.btoa(row.item_fg_number) + "&filter_item_rm_id=" + window.btoa(filter_item_rm_id),
                    singleSelect: true,
                    rownumbers: true,
                    columns: [
                        [{
                ...
                handler: function() {
                    ...
                    for (let i = 0; i < totalrows; i++) {
                        if (rows[i].item_rm_id) {
                            $.ajax({
                                type: "post",
                                url: '<?= base_url('master/bom_compound/create') ?>',
                                data: {
    ...
    $('#dlg_upload').dialog({
        buttons: [{
            text: 'List Failed',
            handler: function() {
                window.open('<?= base_url('master/bom_compound/uploadDownloadFailed') ?>', '_blank');
            }
        }, {
            text: 'Upload',
            iconCls: 'icon-ok',
            handler: function() {
                $('#frm_upload').form('submit', {
                    url: '<?= base_url('master/bom_compound/upload') ?>',
                    onSubmit: function() {
                    ...
                    success: function(result) {
                        ...
                        //Clear File
                        $.ajax({
                            url: "<?= base_url('master/bom_compound/uploadclearFailed') ?>"
                        });
                        ...
                        function requestData(total, json, number = 1, value = 0, success = 1, failed = 1) {
                            if (value < 100) {
                                ...
                                $.ajax({
                                    type: "POST",
                                    async: true,
                                    url: "<?= base_url('master/bom_compound/uploadCreate') ?>",
                                    data: {
                                        "data": json[number - 1]
                                    },
                                    cache: false,
                                    ...
                                    success: function(result) {
                                        if (result.theme == "success") {
                                            ...
                                            $.ajax({
                                                type: "POST",
                                                async: true,
                                                url: "<?= base_url('master/bom_compound/uploadcreateFailed') ?>",
                                                data: {
                                                    data: json[number - 1],
