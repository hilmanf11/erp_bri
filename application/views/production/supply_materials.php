...
<!-- PDF -->
<iframe id="printout" src="<?= base_url('production/supply_materials/print') ?>" style="width: 100%;" hidden></iframe>
<script>
    //Add Data
    function add() {
    ...
    function request_no(reqDate = "") {
        ...
        $.ajax({
            type: "post",
            url: "<?= base_url('production/supply_materials/request_no') ?>/" + window.btoa(request_date),
            dataType: "html",
            ...
            columns: [
                [{
                    ...
                    editor: {
                        type: 'combogrid',
                        options: {
                            url: '<?= base_url('master/items/readSupply') ?>',
                            required: true,
                            panelWidth: 400,
                            idField: 'number',
                            textField: 'number',
            ...
            $.messager.confirm('Warning', 'Are you sure you want to delete this data?', function(r) {
                if (r) {
                    for (var i = 0; i < rows.length; i++) {
                        ...
                        $.ajax({
                            method: 'post',
                            url: '<?= base_url('production/supply_materials/delete') ?>',
                            data: {
                                id: row.id,
    ...
    function filter() {
        ...
        $('#dg').treegrid({
            url: '<?= base_url('production/supply_materials/datatables') ?>' + url
        });
        $("#printout").contents().find('html').html("<center><br><br><br><b style='font-size:20px;'>Please Wait...</b></center>");
        $("#printout").attr('src', '<?= base_url('production/supply_materials/print') ?>' + url);
    }
    ...
    function excel() {
        ...
        var filter_request_no = $("#filter_request_no").combobox('getValue');
        url = "?filter_period=" + filter_period + "&filter_wp=" + filter_wp + "&filter_request_no=" + filter_request_no;
        window.location.assign('<?= base_url('production/supply_materials/print/excel') ?>' + url);
    }
    ...
    function print_kanban() {
        ...
        if (request_no == "") {
            ...
            window.open("<?= base_url('production/supply_materials/print_kanban/') ?>" + window.btoa(request_no), "_blank");
        }
    }
    ...
    $(function() {
        addTable();
        $('#dg').treegrid({
            url: '<?= base_url('production/supply_materials/datatables') ?>',
            pagination: true,
            rownumbers: true,
            idField: 'id',
                    ...
                    if (period == "" || wp == "" || totalrows <= 0) {
                        ...
                        for (let i = 0; i < totalrows; i++) {
                            if (rows[i].item_id) {
                                $.ajax({
                                    type: "post",
                                    url: '<?= base_url('production/supply_materials/create') ?>',
                                    data: {
        ...
        $("#filter_period").combobox({
            url: '<?= base_url('production/supply_materials/readPeriod') ?>',
            valueField: 'period',
            textField: 'period',
            prompt: "Choose period",
            icons: [{
            ...
            onSelect: function(period) {
                $("#filter_wp").combobox({
                    url: '<?= base_url('production/supply_materials/readWp/') ?>' + period.period,
                    valueField: 'wp',
                    textField: 'wp',
                    prompt: "Choose WP",
                    icons: [{
                    ...
                    onSelect: function(wp) {
                        $("#filter_request_no").combobox({
                            url: '<?= base_url('production/supply_materials/readRequestNo/') ?>' + period.period + '/' + window.btoa(wp.wp),
    ...
    function BtnPrintLabel(val, row) {
        return '<a style="text-decoration: none; font-weight:bold;" target="_blank" href="<?= base_url('production/supply_materials/print_label/') ?>' + window.btoa(row.id) + '"><i class="fa fa-print"></i> Print</a>';
    }
</script>