...
<!-- PDF -->
<iframe id="printout" src="<?= base_url('production/checksheets/print') ?>" style="width: 100%;" hidden></iframe>
<script>
    //Add Data
    function add() {
        $('#dlg_insert').dialog('open');
        url_save = '<?= base_url('production/checksheets/create') ?>';
        $('#frm_insert').form('clear');
        $("#trans_date").datebox('setValue', "<?= date("Y-m-d") ?>");

        $('#workorder').combogrid({
            url: '<?= base_url('production/checksheets/readWorkorder') ?>',
            panelWidth: 350,
            idField: 'workorder',
            textField: 'workorder',
            mode: 'remote',
            ...
            $.messager.confirm('Warning', 'Are you sure you want to delete this data?', function(r) {
                if (r) {
                    for (var i = 0; i < rows.length; i++) {
                        ...
                        $.ajax({
                            method: 'post',
                            url: '<?= base_url('production/checksheets/delete') ?>',
                            data: {
                                id: row.id,
    ...
    function filter() {
        ...
        $('#dg').datagrid({
            url: '<?= base_url('production/checksheets/datatables') ?>' + url
        });
        $("#printout").contents().find('html').html("<center><br><br><br><b style='font-size:20px;'>Please Wait...</b></center>");
        $("#printout").attr('src', '<?= base_url('production/checksheets/print') ?>' + url);
    }
    ...
    function excel() {
        ...
        window.location.assign('<?= base_url('production/checksheets/print/excel') ?>' + url);
    }
    ...
    $(function() {
        $('#dg').datagrid({
            url: '<?= base_url('production/checksheets/datatables') ?>',
            pagination: true,
            rownumbers: true
        });
        //Save Data
    ...
    function filter_workorder() {
        ...
        $('#filter_workorder').combogrid({
            url: '<?= base_url('production/checksheets/readWorkorder/filter') ?>',
            panelWidth: 300,
            idField: 'workorder',
            textField: 'workorder',
            mode: 'remote',
    ...
    function BtnPrint(val, row) {
        return '<a class="btn btn-primary w-100" style="pointer-events: visible; opacity:1;" target="_blank" href="<?= base_url('production/checksheets/print_label/') ?>' + window.btoa(row.id) + '"><i class="fa fa-print"></i> Print</a>';
    }
