...
<table id="dg" class="easyui-datagrid" style="width:100%;" toolbar="#toolbar">
    <thead>
        <tr>
            ...
            <th data-options="field:'source',width:100,halign:'center',align:'center'">Source</th>
...
<div id="toolbar" style="height: 250px;">
    <div style="width: 100%; padding: 10px;">
        ...
        <fieldset style="width: 100%; border:2px solid #d0d0d0; margin-bottom: 5px; margin-top: 5px; border-radius:4px;">
            <legend style="text-align: center;"><b>Scan In Rework</b></legend>
...
<div id="dlgRework" class="easyui-dialog" title="Summary WIP Store Scan In Rework" style="width:1000px;height:500px;padding:10px" closed="true" modal="true" buttons="#dlgReworkButtons">

    <table id="dgRework" class="easyui-datagrid" style="width:100%;height:100%;"></table>
</div>

<div id="dlgReworkButtons">
    <a href="javascript:;" class="easyui-linkbutton" iconCls="icon-no" onclick="$('#dlgRework').dialog('close')">Close</a>
...
<script>
    ...
    $(function() {
        ...
        setTimeout(function() {
            ...
            $('#dg').datagrid({
                url: '<?= base_url("control/scan_in_rework/getScanInRework") ?>',
                rownumbers: true,
        ...
        $('#serial_label').keypress(function(e) {
            if (e.which == 13) {
                ...
                $.ajax({
                    type: "POST",
                    url: "<?= base_url('control/scan_in_rework/getChecksheetLabel') ?>",
                    ...
                    success: function(json) {
                        ...
                        if (json.title === "success") {
                            ...
                            $.ajax({
                                type: "POST",
                                url: "<?= base_url('control/scan_in_rework/create') ?>",
                                data: {
    ...
    function btnPreview() {
        ...
        $('#dlgRework').dialog('open').dialog('center');

        $('#dgRework').datagrid({
            url: '<?= base_url('control/scan_in_rework/getSummary') ?>',
            method: 'get',
            fitColumns: true,
            singleSelect: true,
            ...
            onLoadSuccess: function (data) {
                ...
                let rows = $('#dgRework').datagrid('getRows');
                if (rows.length && rows[rows.length - 1].is_total) {
                    $('#dgRework').datagrid('deleteRow', rows.length - 1);
                }

                $('#dgRework').datagrid('appendRow', {
                    is_total: true,
                    qty_total: totalQty
                });
            }
    ...
    function saveSummary() {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                ...
                $.ajax({
                    type: 'POST',
                    url: '<?= base_url('control/scan_in_rework/saveSummary') ?>',
                    data: { items: items },
                    dataType: 'json',
                    beforeSend: function () {
                        Swal.fire({
