...
<!-- Insert & Update -->
<div id="dlg_insert" class="easyui-dialog" title="Add New" data-options="closed: true,modal:true" style="width: 1500px; height: 100%; padding:10px; top: 20px;">
...
<!-- PDF -->
<iframe id="printout" src="<?= base_url('control/label_packing/print') ?>" style="width: 100%;" hidden></iframe>
<script>
    ...
    function request_no(reqDate = "") {
        ...
        $.ajax({
            type: "post",
            url: "<?= base_url('control/label_packing/request_no') ?>/" + window.btoa(request_date),
            dataType: "html",
    ...
    function filter() {
        ...
        $('#dg').treegrid({
            url: '<?= base_url('control/label_packing/datatables') ?>',
            method: 'post',
            queryParams: {
    ...
    function excel() {
        ...
        window.location.assign('<?= base_url('control/label_packing/print/excel') ?>' + url);
    }
                            ...
                            $.messager.confirm('Warning', 'Are you sure you want to save this data?', function (r) {
                                if (r) {
                                    ...
                                    $.each(rows, function (index, row) {
                                        ...
                                        $.ajax({
                                            type: "POST",
                                            url: '<?= base_url('control/label_packing/create') ?>',
                                            data: {
