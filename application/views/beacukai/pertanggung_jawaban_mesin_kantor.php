...
<script>
    ...
    function filter() {
        ...
        $("#printout").attr('src', '<?= base_url('beacukai/pertanggung_jawaban_mesin_kantor/print') ?>' + url);
    }
    ...
    function excel() {
        ...
        url = "?filter_from=" + filter_from + "&filter_to=" + filter_to + "&filter_items=" + filter_items;
        window.location.assign('<?= base_url('beacukai/pertanggung_jawaban_mesin_kantor/print/excel') ?>' + url);
    }

    $(function() {
        $('#filter_items').combogrid({
            url: '<?= base_url('beacukai/pertanggung_jawaban_mesin_kantor/readItems') ?>',
            panelWidth: 420,
            idField: 'id',
            textField: 'number',
            mode: 'remote',
