...
<script>
    ...
    function filter() {
        ...
        $("#printout").attr('src', '<?= base_url('beacukai/pertanggung_jawaban_penolong/print') ?>' + url);
    }
    ...
    function excel() {
        ...
        url = "?filter_from=" + filter_from + "&filter_to=" + filter_to + "&filter_items=" + filter_items;
        window.location.assign('<?= base_url('beacukai/pertanggung_jawaban_penolong/print/excel') ?>' + url);
    }

    $(function() {
        $('#filter_items').combogrid({
            url: '<?= base_url('beacukai/pertanggung_jawaban_penolong/readItems') ?>',
            panelWidth: 420,
            idField: 'id',
            textField: 'number',
            mode: 'remote',
