<table id="dg" class="easyui-datagrid" style="width:100%;" toolbar="#toolbar" data-options="fit: true">
    <thead>
        <tr>
            ...
            <th rowspan="2"data-options="field:'qty',width:100,halign:'center',align:'right', formatter:numberformat">Stock</th>
            <th rowspan="2" data-options="field:'actual',width:100,halign:'center',align:'right',editor:'numberbox', formatter:numberformat">Actual</th>
        ...
        <tr>
            ...
            <th data-options="field:'created_date',width:100,halign:'center'">Date</th>
            <th data-options="field:'updated_by',width:100,halign:'center'">By</th>
            <th data-options="field:'updated_date',width:100,halign:'center'">Date</th>
        </tr>
    </thead>
</table>
        ...
        <fieldset style="width: 60%; border:2px solid #d0d0d0; margin-bottom: 5px; margin-top: 5px; border-radius:4px;">
            ...
            <div style="width: 50%; float: left;">
                ...
                <div class="fitem">
                    ...
                    <input style="width:60%;" id="filter_cutoff" class="easyui-datebox" value="<?= date("Y-m-t") ?>" data-options="formatter:myformatter,parser:myparser, editable:false">
                </div>
                <div class="fitem">
...
<script>
    ...
    function add() {
        ...
        if (rows.length > 0) {
            $.messager.confirm('Warning', 'Are you sure you want to save this data?', function(r) {
                if (r) {
                    $.ajax({
                        url: "<?= base_url('planning/rm_stock/uploadclearFailed') ?>"
                    });
                    ...
                    function requestData(total, json, number = 1, value = 0, success = 1, failed = 1) {
                        if (value < 100) {
                            ...
                            $('#p_start').html(number);
                            $('#p_finish').html(total);
                            $.post('<?= base_url('planning/rm_stock/create') ?>', {
                                data: json[number - 1]
                            }, function(note) {
                                ...
                                if (result.theme == "success") {
                                    ...
                                    $.ajax({
                                        type: "POST",
                                        async: true,
                                        url: "<?= base_url('planning/rm_stock/uploadcreateFailed') ?>",
                                        data: {
                                            data: json[number - 1],
    ...
    //EDIT DATA
    function update() {
        toastr.info("Not Available this function");
    }
    //Delete Data
    function deleted() {
            ...
            $.messager.confirm('Warning', 'Are you sure you want to delete this data?', function(r) {
                if (r) {
                    for (var i = 0; i < rows.length; i++) {
                        ...
                        $.ajax({
                            method: 'post',
                            url: '<?= base_url('planning/rm_stock/delete') ?>',
                            data: {
    ...
    function filter() {
        ...
        if (filter_month == "" || filter_year == "" || filter_revision == "" || filter_cutoff == "") {
            ...
            $("#printout").attr('src', '<?= base_url('planning/rm_stock/print') ?>' + url);
            $('#dg').datagrid({
                rownumbers: true,
                url: '<?= base_url('planning/rm_stock/datatables') ?>' + url
    ...
    function excel() {
        ...
        if (filter_month == "" || filter_year == "" || filter_revision == "" || filter_cutoff == "") {
            ...
            window.location.assign('<?= base_url('planning/rm_stock/print/excel') ?>' + url);
        }
    }
    //Upload Data
    function upload() {
    ...
    function download_excel() {
        window.location.assign('<?= base_url('template/tmp_rm_stock.xls') ?>');
    }
    function reload() {
    ...
    $(function() {
        ...
        $('#dlg_upload').dialog({
            buttons: [{
                text: 'List Failed',
                handler: function() {
                    window.open('<?= base_url('planning/rm_stock/uploadDownloadFailed') ?>', '_blank');
                }
            }, {
                text: 'Upload',
                iconCls: 'icon-ok',
                handler: function() {
                    $('#frm_upload').form('submit', {
                        url: '<?= base_url('planning/rm_stock/upload') ?>',
                        onSubmit: function() {
                            ...
                            function requestData(total, json, number = 1, value = 0, success = 1, failed = 1) {
                                if (value < 100) {
                                    ...
                                    $.ajax({
                                        type: "POST",
                                        async: true,
                                        url: "<?= base_url('planning/rm_stock/uploadCreate') ?>",
                                        data: {
                                            "data": json[number - 1]
                                        },
                                        cache: false,
        ...
        $('#filter_month').combobox({
            ...
            onChange: function(row) {
                ...
                $('#filter_revision').combobox({
                    url: '<?php echo base_url('planning/rm_stock/readRevision?month='); ?>' + month + '&year=' + year,
                    valueField: 'revision',
                    textField: 'revision',
        ...
        $('#filter_product_family').combobox({
            url: '<?php echo base_url('planning/rm_stock/readProductFamily'); ?>',
            valueField: 'pfm_id',
            textField: 'pfm_name',
            ...
            onSelect: function(row){
                $('#filter_product_no').combogrid({
                    url: '<?= base_url('planning/rm_stock/readProducts/') ?>' + row.pfm_id,
                    panelWidth: 400,
                    idField: 'item_id',
                    textField: 'item_id',
                    mode: 'remote',
