<table id="dg" class="easyui-datagrid" style="width:100%;" toolbar="#toolbar" data-options="fit: true">
    <thead>
        <tr>
            ...
            <th rowspan="2" data-options="field:'qty',width:100,halign:'center',align:'right', formatter:numberformat">OS Supply</th>
...
<script>
    ...
    function add() {
        ...
        if (rows.length > 0) {
            $.messager.confirm('Warning', 'Are you sure you want to save this data?', function(r) {
                if (r) {
                    $.ajax({
                        url: "<?= base_url('planning/os_supply/uploadclearFailed') ?>"
                    });
                    ...
                    function requestData(total, json, number = 1, value = 0, success = 1, failed = 1) {
                        if (value < 100) {
                            ...
                            $('#p_start').html(number);
                            $('#p_finish').html(total);
                            $.post('<?= base_url('planning/os_supply/create') ?>', {
                                data: json[number - 1]
                            }, function(note) {
                                ...
                                if (result.theme == "success") {
                                    ...
                                    $.ajax({
                                        type: "POST",
                                        async: true,
                                        url: "<?= base_url('planning/os_supply/uploadcreateFailed') ?>",
                                        data: {
                                            data: json[number - 1],
            ...
            $.messager.confirm('Warning', 'Are you sure you want to delete this data?', function(r) {
                if (r) {
                    for (var i = 0; i < rows.length; i++) {
                        ...
                        $.ajax({
                            method: 'post',
                            url: '<?= base_url('planning/os_supply/delete') ?>',
                            data: {
    ...
    function filter() {
        ...
        if (filter_month == "" || filter_year == "" || filter_revision == "" || filter_cutoff == "") {
            ...
            $("#printout").attr('src', '<?= base_url('planning/os_supply/print') ?>' + url);
            $('#dg').datagrid({
                rownumbers: true,
                url: '<?= base_url('planning/os_supply/datatables') ?>' + url
    ...
    function excel() {
        ...
        if (filter_month == "" || filter_year == "" || filter_revision == "" || filter_cutoff == "") {
            ...
            window.location.assign('<?= base_url('planning/os_supply/print/excel') ?>' + url);
        }
    }
    //Upload Data
    function upload() {
    ...
    function download_excel() {
        window.location.assign('<?= base_url('template/tmp_os_supply.xls') ?>');
    }
    function reload() {
    ...
    $(function() {
        ...
        $('#dlg_upload').dialog({
            buttons: [{
                text: 'List Failed',
                handler: function() {
                    window.open('<?= base_url('planning/os_supply/uploadDownloadFailed') ?>', '_blank');
                }
            }, {
                text: 'Upload',
                iconCls: 'icon-ok',
                handler: function() {
                    $('#frm_upload').form('submit', {
                        url: '<?= base_url('planning/os_supply/upload') ?>',
                        onSubmit: function() {
                            ...
                            function requestData(total, json, number = 1, value = 0, success = 1, failed = 1) {
                                if (value < 100) {
                                    ...
                                    $.ajax({
                                        type: "POST",
                                        async: true,
                                        url: "<?= base_url('planning/os_supply/uploadCreate') ?>",
                                        data: {
                                            "data": json[number - 1]
                                        },
                                        cache: false,
        ...
        $('#filter_month').combobox({
            ...
            onChange: function(row) {
                ...
                $('#filter_revision').combobox({
                    url: '<?php echo base_url('planning/os_supply/readRevision?month='); ?>' + month + '&year=' + year,
                    valueField: 'revision',
                    textField: 'revision',
        ...
        $('#filter_product_family').combobox({
            url: '<?php echo base_url('planning/os_supply/readProductFamily'); ?>',
            valueField: 'pfm_id',
            textField: 'pfm_name',
            ...
            onSelect: function(row){
                $('#filter_product_no').combogrid({
                    url: '<?= base_url('planning/os_supply/readProducts/') ?>' + row.pfm_id,
                    panelWidth: 400,
                    idField: 'item_id',
                    textField: 'item_id',
                    mode: 'remote',
