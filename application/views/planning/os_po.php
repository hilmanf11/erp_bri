...
<table id="dg" class="easyui-datagrid" style="width:100%;" toolbar="#toolbar">
    <thead>
        <tr>
            ...
            <th rowspan="2" data-options="field:'po_no',width:150,halign:'center'">Purchase Order No.</th>
            <th rowspan="2" data-options="field:'po_date',width:100,halign:'center'">Po Date</th>
            <th rowspan="2" data-options="field:'supplier_id',width:100,halign:'center'">Supplier ID</th>
            <th rowspan="2" data-options="field:'supplier_name',width:200,halign:'center'">Supplier Name</th>
            <th rowspan="2" data-options="field:'item_rm_id',width:100,halign:'center'">Part ID</th>
...
<div id="dlg_insert" class="easyui-dialog" title="Add New" data-options="closed: true,modal:true" style="width: 500px; padding:10px; top: 20px;">
    <form id="frm_insert" method="post" novalidate>
        <fieldset style="width:100%; border:1px solid #d0d0d0; margin-bottom: 10px; border-radius:4px; float: left;">
            ...
            <div class="fitem">
                <span style="width:35%; display:inline-block;">Po No</span>
                <input style="width:60%;" name="po_no" id="po_no" required class="easyui-textbox">
            </div>
            <div class="fitem">
...
<!-- PDF -->
<iframe id="printout" src="<?= base_url('planning/os_po/print') ?>" style="width: 100%;" hidden></iframe>
...
<script>
    ...
    function add() {
        ...
        url_save = '<?= base_url('planning/os_po/create') ?>';
    ...
    function update() {
        ...
        if (row) {
            ...
            url_save = '<?= base_url('planning/os_po/update') ?>?id=' + btoa(row.id);
        } else {
            ...
            $.messager.confirm('Warning', 'Are you sure you want to delete this data?', function(r) {
                if (r) {
                    for (var i = 0; i < rows.length; i++) {
                        ...
                        $.ajax({
                            method: 'post',
                            url: '<?= base_url('planning/os_po/delete') ?>',
                            data: {
                                id: row.id,
                            },
    ...
    function download_excel() {
        window.location.assign('<?= base_url('template/tmp_os_po.xls') ?>');
    }
    ...
    function filter() {
        ...
        $('#dg').datagrid({
            url: '<?= base_url('planning/os_po/datatables') ?>' + url,
            pagination: true,
            clientPaging: false,
            remoteFilter: true,
            rownumbers: true,
        ...
        $("#printout").attr('src', '<?= base_url('planning/os_po/print') ?>' + url);
    }
    ...
    $('#dlg_upload').dialog({
        buttons: [{
            text: 'List Failed',
            handler: function() {
                window.open('<?= base_url('planning/os_po/uploadDownloadFailed') ?>', '_blank');
            }
        }, {
            text: 'Upload',
            iconCls: 'icon-ok',
            handler: function() {
                $('#frm_upload').form('submit', {
                    url: '<?= base_url('planning/os_po/upload') ?>',
                    onSubmit: function() {
                    ...
                    success: function(result) {
                        ...
                        //Clear File
                        $.ajax({
                            url: "<?= base_url('planning/os_po/uploadclearFailed') ?>"
                        });
                        ...
                        function requestData(total, json, number = 1, value = 0, success = 1, failed = 1) {
                            if (value < 100) {
                                ...
                                $.ajax({
                                    type: "POST",
                                    async: true,
                                    url: "<?= base_url('planning/os_po/uploadCreate') ?>",
                                    data: {
                                        "data": json[number - 1]
                                    },
                                    cache: false,
                                    ...
                                    success: function(result) {
                                        if (result.theme == "success") {
                                            ...
                                            $.ajax({
                                                type: "POST",
                                                async: true,
                                                url: "<?= base_url('planning/os_po/uploadcreateFailed') ?>",
                                                data: {
                                                    data: json[number - 1],
