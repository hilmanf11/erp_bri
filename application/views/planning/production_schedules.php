    ...
    <fieldset style="width: 70%; border:2px solid #d0d0d0; margin-bottom: 5px; margin-top: 5px; border-radius:4px;">
        ...
        <div style="width: 50%; float: left;">
            ...
            <div class="fitem">
                ...
                <input style="width:60%;" id="filter_lines" class="easyui-combobox" panelHeight="auto" data-options="prompt:'Select Line Production'">
            </div>
            <div class="fitem">
    ...
    <form id="frm_insert" method="post" novalidate>
        <fieldset style="width:100%; border:1px solid #d0d0d0; margin-bottom: 10px; border-radius:4px; float: left;">
            ...
            <div class="fitem">
                ...
                <input style="width:60%;" name="item_id" required="" id="item_id" required="" class="easyui-combogrid">
            </div>
            <div class="fitem">
...
<script>
    ...
    function filter() {
        ...
        var filter_lines = $("#filter_lines").combobox('getValue');
        var filter_customers = $("#filter_customers").combogrid('getValue');
        var filter_sales_order = $("#filter_sales_order").combobox('getValue');

        url = "?filter_month=" + filter_month + "&filter_year=" + filter_year + "&filter_lines=" + filter_lines + "&filter_customers=" + filter_customers + "&filter_sales_order=" + filter_sales_order;
        $('#dg').datagrid({
    ...
    $(function() {
        ...
        //Get Customer
        $("#filter_lines").combobox({
            url: '<?= base_url('master/lines/reads') ?>',
            valueField: 'id',
            textField: 'name',
            prompt: "Select Line",
            icons: [{
        ...
        $('#filter_customers').combogrid({
            ...
            onSelect: function(val, row) {
                $("#filter_sales_order").combobox({
                    url: '<?= base_url('planning/sales_orders/readSalesOrders/1?customer_id=') ?>' + row.id,
                    valueField: 'number',
                    textField: 'number',
        ...
        $('#customer_id').combogrid({
            ...
            onSelect: function(valCust, rowCust) {
                $('#so_number').combogrid({
                    url: '<?= base_url('planning/sales_orders/readSalesOrders?customer_id=') ?>' + rowCust.id,
                    panelWidth: 420,
                    idField: 'number',
                    textField: 'number',
                    mode: 'remote',
                    ...
                    onSelect: function(valSo, rowSo) {
                        $("#so_date").datebox('setValue', rowSo.trans_date);
                        $('#item_id').combogrid({
                            url: '<?= base_url('planning/sales_orders/readItems?customer_id=') ?>' + rowCust.id + "&sales_order=" + rowSo.number,
                            panelWidth: 500,
                            idField: 'item_id',
