<table id="dg" class="easyui-datagrid" style="width:100%;" toolbar="#toolbar">
    <thead>
        <tr>
            ...
            <th rowspan="2" data-options="field:'product_name',width:150,halign:'center'">Product Name</th>
            <th rowspan="2" data-options="field:'product_family',width:150,halign:'center'">Product Family</th>
            <th rowspan="2" data-options="field:'qty',width:100,halign:'center',align:'right', formatter:numberformat">OS Workorder</th>
...
<script>
    ...
    function add() {
        ...
        if (rows.length > 0) {
            $.messager.confirm('Warning', 'Are you sure you want to save this data?', function(r) {
                if (r) {
                    $.ajax({
                        url: "<?= base_url('planning/os_mppp/uploadclearFailed') ?>"
                    });
                    ...
                    function requestData(total, json, number = 1, value = 0, success = 1, failed = 1) {
                        if (value < 100) {
                            ...
                            $('#p_start').html(number);
                            $('#p_finish').html(total);
                            $.post('<?= base_url('planning/os_mppp/create') ?>', {
                                data: json[number - 1]
                            }, function(note) {
                                ...
                                if (result.theme == "success") {
                                    ...
                                    $.ajax({
                                        type: "POST",
                                        async: true,
                                        url: "<?= base_url('planning/os_mppp/uploadcreateFailed') ?>",
                                        data: {
                                            data: json[number - 1],
            ...
            $.messager.confirm('Warning', 'Are you sure you want to delete this data?', function(r) {
                if (r) {
                    for (var i = 0; i < rows.length; i++) {
                        ...
                        $.ajax({
                            method: 'post',
                            url: '<?= base_url('planning/os_mppp/delete') ?>',
                            data: {
    ...
    function filter() {
        ...
        if (filter_month == "" || filter_year == "" || filter_revision == "" || filter_cutoff == "") {
            ...
            $("#printout").attr('src', '<?= base_url('planning/os_mppp/print') ?>' + url);
            $('#dg').datagrid({
                rownumbers: true,
                url: '<?= base_url('planning/os_mppp/datatables') ?>' + url
    ...
    function excel() {
        ...
        if (filter_month == "" || filter_year == "" || filter_revision == "" || filter_cutoff == "") {
            ...
            window.location.assign('<?= base_url('planning/os_mppp/print/excel') ?>' + url);
        }
    }
    //Upload Data
    function upload() {
    ...
    function download_excel() {
        window.location.assign('<?= base_url('template/tmp_os_mppp.xls') ?>');
    }
    function reload() {
    ...
    $(function() {
        ...
        $('#dlg_upload').dialog({
            buttons: [{
                text: 'List Failed',
                handler: function() {
                    window.open('<?= base_url('planning/os_mppp/uploadDownloadFailed') ?>', '_blank');
                }
            }, {
                text: 'Upload',
                iconCls: 'icon-ok',
                handler: function() {
                    $('#frm_upload').form('submit', {
                        url: '<?= base_url('planning/os_mppp/upload') ?>',
                        onSubmit: function() {
                            ...
                            function requestData(total, json, number = 1, value = 0, success = 1, failed = 1) {
                                if (value < 100) {
                                    ...
                                    $.ajax({
                                        type: "POST",
                                        async: true,
                                        url: "<?= base_url('planning/os_mppp/uploadCreate') ?>",
                                        data: {
                                            "data": json[number - 1]
                                        },
                                        cache: false,
        ...
        $('#filter_month').combobox({
            ...
            onChange: function(row) {
                ...
                $('#filter_revision').combobox({
                    url: '<?php echo base_url('planning/os_mppp/readRevision?month='); ?>' + month + '&year=' + year,
                    valueField: 'revision',
                    textField: 'revision',
        ...
        $('#filter_product_family').combobox({
            url: '<?php echo base_url('planning/os_mppp/readProductFamily'); ?>',
            valueField: 'pfm_id',
            textField: 'pfm_name',
            ...
            onSelect: function(row){
                $('#filter_product_no').combogrid({
                    url: '<?= base_url('planning/os_mppp/readProducts/') ?>' + row.pfm_id,
                    panelWidth: 400,
                    idField: 'item_id',
                    textField: 'item_id',
                    mode: 'remote',
