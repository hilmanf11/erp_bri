...
<!-- Insert -->
<div id="dlg_insert" class="easyui-dialog" title="Add Packing List" data-options="closed: true,modal:true" style="width: 1000px; height: 600px; padding:10px; top: 20px;">
...
<!-- PDF -->
<iframe id="printout" src="<?= base_url('shipping/packing_lists/print') ?>" style="width: 100%;" hidden></iframe>
<script>
    //Add Data
    function add() {
        ...
        $("#dn_number").combobox({
            url: '<?= base_url('shipping/packing_lists/readDeliveryNote') ?>',
            valueField: 'number',
            textField: 'number',
            prompt: "Choose Delivery Note",
            onSelect: function(dn_number) {
                $("#trans_type").textbox('setValue', dn_number.trans_type);
                $("#remarks").textbox('setValue', dn_number.note);
            }
        });
    }
    ...
    function number(trans_date) {
        $.ajax({
            type: "post",
            url: "<?= base_url('shipping/packing_lists/number/') ?>" + window.btoa(trans_date),
            dataType: "html",
    ...
    function preview() {
        ...
        if (dn_number == "") {
            ...
            var lastIndex;
            $('#dg_request').datagrid({
                url: '<?= base_url('shipping/delivery_notes/reads') ?>?number=' + dn_number,
                columns: [
                    [{
                        field: 'ck',
                        checkbox: true,
                ...
                if (r) {
                    for (var i = 0; i < rows.length; i++) {
                        ...
                        if (row.state != "closed") {
                            $.ajax({
                                method: 'post',
                                url: '<?= base_url('shipping/packing_lists/delete') ?>',
                                data: {
                                    id: row.id,
    ...
    function filter() {
        ...
        $('#dg').treegrid({
            url: '<?= base_url('shipping/packing_lists/datatables') ?>' + url
        });
        $("#printout").contents().find('html').html("<center><br><br><br><b style='font-size:20px;'>Please Wait...</b></center>");
        $("#printout").attr('src', '<?= base_url('shipping/packing_lists/print') ?>' + url);
    }
    ...
    function excel() {
        ...
        var filter_customer = $("#filter_customer").combobox('getValue');
        url = "?filter_from=" + filter_from + "&filter_to=" + filter_to + "&filter_packing_list=" + filter_packing_list + "&filter_customer=" + filter_customer;
        window.location.assign('<?= base_url('shipping/packing_lists/print/excel') ?>' + url);
    }
    ...
    function print_dn() {
        ...
        if (dn_no == "") {
            ...
            window.open("<?= base_url('shipping/packing_lists/print_dn/') ?>" + window.btoa(dn_no), "_blank");
        }
    }
    ...
    $(function() {
        ...
        $('#dg').treegrid({
            url: '<?= base_url('shipping/packing_lists/datatables') ?>',
            pagination: true,
            rownumbers: true,
            idField: 'id',
            treeField: 'number',
                        ...
                        $.messager.confirm('Warning', 'Are you sure you want to Create Packing Lists?', function(r) {
                            if (r) {
                                for (var i = 0; i < rows.length; i++) {
                                    ...
                                    $.ajax({
                                        type: "post",
                                        url: '<?= base_url('shipping/packing_lists/create') ?>',
                                        data: 'number=' + number +
        ...
        $("#filter_customer").combobox({
            ...
            onSelect: function(customer) {
                $("#filter_packing_list").combobox({
                    url: '<?= base_url('shipping/packing_lists/readPackingLists/') ?>' + customer.id,
                    valueField: 'number',
                    textField: 'number',
