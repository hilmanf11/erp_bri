...
<table id="dg" class="easyui-datagrid" style="width:100%;" toolbar="#toolbar">
    <thead>
        <tr>
            ...
            <th rowspan="2" data-options="field:'number',halign:'center',width:150">Delivery Order No</th>
...
<div id="toolbar" style="height: 320px;">
    ...
    <div style="width: 100%; padding: 10px;">
        <fieldset style="width: 100%; border:2px solid #d0d0d0; margin-bottom: 5px; margin-top: 5px; border-radius:4px;">
            ...
            <div class="fitem" style="padding:0 200px 0 200px;">
                <input style="width:100%; height: 80px;" type="text" id="do_number" name="do_number" class="scan" placeholder="SCAN DELIVERY ORDER HERE">
            </div>
            <div class="fitem" style="padding:0 200px 0 200px;">
...
<script>
    ...
    $(function() {
        ...
        //Scan Supply Sheet
        $('#do_number').focus();
        $('#do_number').keypress(function(e) {
            if (e.which == 13) {
                var do_number = $(this).val();

                $.ajax({
                    type: "GET",
                    url: "<?= base_url('shipping/shipping_orders/getDeliveryOrders') ?>",
                    data: "do_number=" + do_number,
                    dataType: "json",
                    success: function(json) {
                        if (json.total > 0) {
                            $('#dg').datagrid({
                                url: '<?= base_url('shipping/shipping_orders/getDeliveryOrders?do_number=') ?>' + do_number,
                                rownumbers: true
                            });
        ...
        $('#checksheet_label').keypress(function(e) {
            if (e.which == 13) {
                ...
                $.ajax({
                    type: "POST",
                    url: "<?= base_url('shipping/shipping_orders/getChecksheetLabel') ?>",
                    data: "checksheet_label=" + checksheet_label + "&do_number=" + do_number,
                    dataType: "json",
                    success: function(json) {
                        if (json.total > 0) {
                            var row = json.rows;
                            for (let i = 0; i < json.total; i++) {
                                $.ajax({
                                    type: "POST",
                                    url: "<?= base_url('shipping/shipping_orders/create') ?>",
