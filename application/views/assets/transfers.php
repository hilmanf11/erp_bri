...
<!-- PDF -->
<iframe id="printout" src="<?= base_url('assets/transfers/print') ?>" style="width: 100%;" hidden></iframe>
<script>
    //ADD DATA
    function add() {
    ...
    function update() {
        ...
        if (row) {
            ...
            addTable('<?= base_url('assets/transfers/datatable_updates?number=') ?>' + window.btoa(row.number));
        } else {
    ...
    function number(transfer_date) {
        $.ajax({
            type: "post",
            url: "<?= base_url('assets/transfers/number/') ?>" + window.btoa(transfer_date),
            dataType: "html",
            ...
            columns: [
                [{
                    ...
                    editor: {
                        type: 'combogrid',
                        options: {
                            url: '<?= base_url('assets/fixeds/reads') ?>',
                            required: true,
                            panelWidth: 400,
                            idField: 'name',
                            textField: 'name',
            ...
            $.messager.confirm('Warning', 'Are you sure you want to delete this data?', function(r) {
                if (r) {
                    for (var i = 0; i < rows.length; i++) {
                        ...
                        $.ajax({
                            method: 'post',
                            url: '<?= base_url('assets/transfers/delete') ?>',
                            data: {
                                number: row.number
                            },
                            success: function(result) {
    ...
    function filter() {
        ...
        $('#dg').datagrid({
            url: '<?= base_url('assets/transfers/datatables') ?>' + url
        });
        $("#printout").contents().find('html').html("<center><br><br><br><b style='font-size:20px;'>Please Wait...</b></center>");
        $("#printout").attr('src', '<?= base_url('assets/transfers/print') ?>' + url);
    }
    //PRINT PDF
    function pdf() {
    ...
    function excel() {
        ...
        var url = "?filter_customers=" + window.btoa(filter_customers) +
            ...
            "&filter_number=" + window.btoa(filter_number) +
            "&filter_type=" + window.btoa(filter_type);
        window.location.assign('<?= base_url('assets/transfers/print/excel') ?>' + url);
    }
    //RELOAD
    function reload() {
    ...
    $(function() {
        ...
        $('#dg').datagrid({
            url: '<?= base_url('assets/transfers/datatables') ?>',
            pagination: true,
            rownumbers: true,
            height: '810px',
            view: detailview,
            ...
            onExpandRow: function(index, row) {
                ...
                var url = "?filter_customers=" + window.btoa(filter_customers) +
                    ...
                    "&filter_type=" + window.btoa(filter_type);
                ddv.datagrid({
                    url: '<?= base_url('assets/transfers/datatables/details?number=') ?>' + window.btoa(row.number) +
                    ...
                    if (customer_id == "" || customer_po == "" || totalrows <= 0) {
                        ...
                        for (let i = 0; i < totalrows; i++) {
                            ...
                            if (rows[i].item_id) {
                                $.ajax({
                                    type: "post",
                                    url: '<?= base_url('assets/transfers/create') ?>',
                                    data: {
        ...
        $("#filter_number").combogrid({
            url: '<?= base_url('assets/transfers/reads') ?>',
            panelWidth: 350,
            idField: 'number',
            textField: 'number',
            mode: 'remote',
