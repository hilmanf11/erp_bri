...
<!-- PDF -->
<iframe id="printout" src="<?= base_url('warehouse/rm_location_items/print') ?>" style="width: 100%;" hidden></iframe>
<script>
    //ADD DATA
    function add() {
        $('#dlg_insert').dialog('open');
        url_save = '<?= base_url('warehouse/rm_location_items/create') ?>';
    ...
    function update() {
        ...
        if (row) {
            ...
            url_save = '<?= base_url('warehouse/rm_location_items/update') ?>?id=' + btoa(row.id);
            $("#location").combobox({
                url: '<?= base_url('warehouse/rm_locations/readLocations') ?>',
                valueField: 'location',
                textField: 'location',
                ...
                onSelect: function(loc) {
                    $("#area").combobox({
                        url: '<?= base_url('warehouse/rm_locations/readAreas?location=') ?>' + loc.location,
                        valueField: 'area',
                        textField: 'area',
                        prompt: "Choose Area",
                        onLoadSuccess: function() {
                        ...
                        onSelect: function(ar) {
                            $("#rack").combobox({
                                url: '<?= base_url('warehouse/rm_locations/readRacks?location=') ?>' + loc.location + "&area=" + ar.area,
                                valueField: 'rack',
                                textField: 'rack',
                                prompt: "Choose Rack",
                                onLoadSuccess: function() {
                                ...
                                onSelect: function(rc) {
                                    $("#level").combobox({
                                        url: '<?= base_url('warehouse/rm_locations/readLevels?location=') ?>' + loc.location + "&area=" + ar.area + "&rack=" + rc.rack,
                                        valueField: 'level',
                                        textField: 'level',
                                        prompt: "Choose Level",
                                        onLoadSuccess: function() {
                                        ...
                                        onSelect: function(lv) {
                                            $("#level_sub").combobox({
                                                url: '<?= base_url('warehouse/rm_locations/readLevelSubs?location=') ?>' + loc.location + "&area=" + ar.area + "&rack=" + rc.rack + "&level=" + lv.level,
            ...
            $.messager.confirm('Warning', 'Are you sure you want to delete this data?', function(r) {
                if (r) {
                    for (var i = 0; i < rows.length; i++) {
                        ...
                        $.ajax({
                            method: 'post',
                            url: '<?= base_url('warehouse/rm_location_items/delete') ?>',
                            data: {
                                id: row.id
                            },
    ...
    //PRINT EXCEL
    function excel() {
        window.location.assign('<?= base_url('warehouse/rm_location_items/print/excel') ?>');
    }
    //RELOAD
    function reload() {
    ...
    $(function() {
        ...
        $('#dg').datagrid({
            url: '<?= base_url('warehouse/rm_location_items/datatables') ?>',
            pagination: true,
            rownumbers: true
        });
        //SAVE DATA
        ...
        $('#dlg_upload').dialog({
            buttons: [{
                text: 'List Failed',
                handler: function() {
                    window.open('<?= base_url('warehouse/rm_location_items/uploadDownloadFailed') ?>', '_blank');
                }
            }, {
                text: 'Upload',
                iconCls: 'icon-ok',
                handler: function() {
                    $('#frm_upload').form('submit', {
                        url: '<?= base_url('warehouse/rm_location_items/upload') ?>',
                        onSubmit: function() {
                        ...
                        success: function(result) {
                            ...
                            //Clear File
                            $.ajax({
                                url: "<?= base_url('warehouse/rm_location_items/uploadclearFailed') ?>"
                            });
                            ...
                            function requestData(total, json, number = 1, value = 0, success = 1, failed = 1) {
                                if (value < 100) {
                                    ...
                                    $.ajax({
                                        type: "POST",
                                        async: true,
                                        url: "<?= base_url('warehouse/rm_location_items/uploadCreate') ?>",
                                        data: {
                                            "data": json[number - 1]
                                        },
                                        cache: false,
                                        ...
                                        success: function(result) {
                                            if (result.theme == "success") {
                                                ...
                                                $.ajax({
                                                    type: "POST",
                                                    async: true,
                                                    url: "<?= base_url('warehouse/rm_location_items/uploadcreateFailed') ?>",
                                                    data: {
                                                        data: json[number - 1],
