...
class Report_history_transactions extends CI_Controller
{
    ...
    public function print($option = "")
    {
        ...
        $records = $this->crud->query("SELECT
            ...
            (COALESCE(SUM(e.qty),0) - COALESCE(f.qty, 0) + COALESCE(g.return_qty, 0) + COALESCE(h.qty_stock_rm, 0) + COALESCE(i.qty_in, 0) - COALESCE(j.qty_out, 0)) as end_stock
        FROM item_rm a 
        ...
        foreach ($records as $record) {
            ...
            $itemReceipts = $this->crud->query("SELECT
                a.id,(COALESCE(SUM(e.qty),0) + COALESCE(g.return_qty, 0) + COALESCE(h.qty_stock_rm, 0)) - (COALESCE(f.qty,0)) as begin_stock   
            FROM item_rm a 
            JOIN item_familys b ON a.item_family_id = b.id and b.number != '006'
