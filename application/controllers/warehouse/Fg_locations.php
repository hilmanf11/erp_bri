...
class Fg_locations extends CI_Controller
{
    ...
    public function index()
    {
        if (empty($this->session->username)) {
            ...
            $this->load->view('warehouse/fg_locations');
        } else {
    ...
    public function readLocations()
    {
        ...
        $send = $this->crud->reads('warehouse_locations', ["location" => $post], ["type" => "FG"], "", "location", "asc", ["location"]);
    ...
    public function readAreas()
    {
        ...
        $send = $this->crud->reads('warehouse_locations', ["area" => $post], ["type" => "FG", "location" => $location], "", "area", "asc", ["area"]);
        echo json_encode($send);
    }
    public function readRacks()
    {
        ...
        $send = $this->crud->reads('warehouse_locations', ["rack" => $post], ["type" => "FG", "location" => $location, "area" => $area], "", "rack", "asc", ["rack"]);
        echo json_encode($send);
    }
    public function readLevels()
    {
        ...
        $send = $this->crud->reads('warehouse_locations', ["level" => $post], ["type" => "FG", "location" => $location, "area" => $area, "rack" => $rack], "", "level", "asc", ["level"]);
        echo json_encode($send);
    }
    public function readLevelSubs()
    {
        ...
        $send = $this->crud->reads('warehouse_locations', ["level_sub" => $post], ["type" => "FG", "location" => $location, "area" => $area, "rack" => $rack, "level" => $level], "", "level_sub", "asc", ["level_sub"]);
        echo json_encode($send);
    }
    //GET DATATABLES
    ...
    public function uploadcreate()
    {
        if ($this->input->post()) {
            ...
            $warehouse_locations = $this->crud->read('warehouse_locations', [], ["number" => $data['number'], "type" => "FG"]);
                ...
                <table style="width: 100%;">
                    <tr>
                        ...
                        <td style="font-size: 14px; text-align: left; margin:2px;">
                            <b>' . $config->name . '</b><br>
                            <small>WAREHOUSE FG LOCATION</small>
                        </td>
                    </tr>
                </table>
            </div>
