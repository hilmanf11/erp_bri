...
class Return_materials extends CI_Controller
{
    ...
    public function datatables()
    {
        ...
        if ($id === "0") {
            ...
            $this->db->from('return_materials a');
            $this->db->join('items b', 'a.item_id = b.id');
            $this->db->join('supplier_items c', 'c.item_id = b.id');
    ...
    public function create()
    {
        if ($this->input->post()) {
            if ($this->form_validation->run() == TRUE) {
                ...
                $return_materials = $this->crud->read('return_materials', [], ["return_no" => $post['return_no'], "item_id" => $post['item_id']]);
    ...
    public function print_label($return_id)
    {
        ...
        $this->db->select('a.*, b.return_date, c.number, c.name, c.description, d.location, d.area');
    ...
    public function print($option = "")
    {
        ...
        $this->db->from('config');
        $config = $this->db->get()->row();
        $this->db->select('a.*, b.number as item_id, b.name as item_name, f.name as uom');
        $this->db->from('return_materials a');
        $this->db->join('items b', 'a.item_id = b.id');
