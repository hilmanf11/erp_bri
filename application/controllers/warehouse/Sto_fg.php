...
class Sto_fg extends CI_Controller
{
    ...
    public function index()
    {
        if (empty($this->session->username)) {
            ...
            $this->load->view('warehouse/sto_fg');
        } else {
    ...
    public function reads()
    {
        ...
        $send = $this->crud->reads('sto_fg', ["name" => $post]);
    ...
    public function create()
    {
        if ($this->input->post()) {
            if ($this->form_validation->run() == TRUE) {
                ...
                $sto_fg = $this->crud->read('sto_fg', [], ["item_id" => $post['item_id'], "process_id" => $post['process_id']]);

                if (!empty($sto_fg->item_id)) {
                    echo json_encode(array("title" => "Duplicated", "message" => "Duplicate Data", "theme" => "error"));
                } else {
                    $send = $this->crud->create('sto_fg', $post);
                    echo $send;
                }
            } else {
    ...
    public function update()
    {
        if ($this->input->post()) {
            ...
            $post = $this->input->post();
            $send = $this->crud->update('sto_fg', ["id" => $id], $post);
            echo $send;
        } else {
            show_error("Cannot Process your request");
        }
    ...
    public function delete()
    {
        ...
        $send = $this->crud->delete('sto_fg', $data);
        echo $send;
    }
    ...
    {
        @unlink('excel/failed/sto_fg.txt');
    }
    ...
    public function uploadcreateFailed()
    {
        if ($this->input->post()) {
            ...
            $textFailed = fopen('excel/failed/sto_fg.txt', 'a');
    ...
    public function uploadcreate()
    {
        if ($this->input->post()) {
            ...
            $sto_fg = $this->crud->read('sto_fg', [], ["item_id" => $item->id, "process_id" => $process->id]);
            ...
            if (empty($item->number)) {
                ...
                $send   = $this->crud->create('sto_fg', $postFinal);
                echo $send;
            }
        }
    }
    ...
    public function print($option = "")
    {
        if ($option == "excel") {
            ...
            header("Content-Disposition: attachment; filename=sto_fg_$format.xls");
        }
        //Config
        $this->db->select('*');
        $this->db->from('config');
                ...
                <table style="width: 100%;">
                    <tr>
                        ...
                        <td style="font-size: 14px; text-align: left; margin:2px;">
                            <b>' . $config->name . '</b><br>
                            <small>STOCK OPNAME FINISH GOOD</small>
                        </td>
                    </tr>
                </table>
            </div>
