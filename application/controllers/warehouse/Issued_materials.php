...
class Issued_materials extends CI_Controller
{
    ...
    public function getPoReceipt()
    {
        if ($this->input->post()) {
            ...
            $this->db->select('a.label_no, b.item_rm_id, a.qty');
    ...
    public function datatables()
    {
        if ($this->input->get()) {
            ...
            //Select Query
            $this->db->select('a.*, b.number as item_number, c.number as item_rm_no, c.name as item_rm_name, c.uom, COALESCE(d.qty_req, 0) as qty_req, f.warehouse, (COALESCE(d.qty_req,0) + f.balance) as balance, (COALESCE(d.qty_req,0) - a.qty) as qty_bal');
