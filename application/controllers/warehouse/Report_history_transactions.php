...
class Report_history_transactions extends CI_Controller
{
    ...
    public function readBalanceWip()
    {
        if ($this->input->post()) {
            $item_id = $this->input->post('item_id');
            $wip_balances = $this->crud->read("wip_balances", [], ["item_id" => $item_id], "", "id", "desc");
    ...
    public function print($option = "")
    {
                    ...
                    <table style="width: 100%;">
                        <tr>
                            ...
                            <td style="font-size: 14px; text-align: left; margin:2px;">
                                <b>' . $config->name . '</b><br>
                                <small>REPORT INVENTORY HISTORY TRANSACTION</small><br>
        ...
        foreach ($records as $record) {
            ...
            $html .= '  <tr>
                            ...
                            <td style="text-align:right;">' . number_format((@$itemReceipts[0]->begin_stock + $record->end_stock), 2) . '</td>
                        </tr>';
            ...
            if ($filter_display == "DETAIL") {
                ...
                for ($i = $start; $i <= $finish; $i += (60 * 60 * 24)) {
                    ...
                    WHERE a.item_id = '$item_id' and a.receipt_date between '$working_date' and '$working_date'
                    GROUP BY a.bc_kind, a.bc_aju, a.bc_document, a.bc_date");

                    //ISSUED
                    $issueds = $this->crud->query("SELECT * FROM issued_material_details WHERE item_id = '$item_id' and DATE_FORMAT(created_date, '%Y-%m-%d') between '$working_date' and '$working_date'");
                    ...
                    WHERE a.item_id = '$item_id' and a.return_date between '$working_date' and '$working_date'
                    GROUP BY b.label_no");
