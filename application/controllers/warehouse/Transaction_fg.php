...
class Transaction_fg extends CI_Controller
{
    ...
    public function index()
    {
        if (empty($this->session->username)) {
            ...
            $this->load->view('warehouse/transaction_fg');
        } else {
    ...
    {
        $records = $this->crud->query("SELECT `period` FROM transaction_fg WHERE `status` = '0' GROUP BY `period`");
        echo json_encode($records);
    }

    public function readWp($period)
    {
        $records = $this->crud->query("SELECT workorder FROM transaction_fg WHERE `status` = '0' and `period` = '$period' GROUP BY workorder");
        echo json_encode($records);
    }
    ...
    public function readRequestNo($period, $workorder)
    {
        ...
        $records = $this->crud->query("SELECT request_no FROM transaction_fg WHERE status = '0' and `period` = '$period' and workorder = '$workorder' GROUP BY `request_no`");
        echo json_encode($records);
    }

    public function readRequestNos()
    {
        $records = $this->crud->query("SELECT request_no FROM transaction_fg WHERE status = '0' GROUP BY `request_no`");
        echo json_encode($records);
    }
    ...
    {
        $records = $this->crud->query("SELECT * FROM transaction_type WHERE type = 'ISFG-001' or type = 'ISFG-002' or type = 'REFG-001' or type = 'REFG-002' or type = 'ISFG-003'");
        echo json_encode($records);
    }
    ...
    public function request_no($trans_date)
    {
        ...
        $sqlGetID   = $this->db->query("SELECT max(request_no) as kode FROM transaction_fg WHERE request_no like '%$datenow%'");
        $rowID      = $sqlGetID->row();
        $kode       = $rowID->kode;
        if ($kode == NULL) {
        ...
        if ($this->input->post()) {
            // if ($this->form_validation->run() == TRUE) {
                ...
                if ($post['qty'] == 0) {
                    ...
                    $datas = $this->crud->reads('transaction_fg', [], ["request_no" => $request_no, "item_fg_id" => $item_fg_id]);

                    if(count($datas) > 0){
                        $send = $this->crud->update('transaction_fg', ["request_no" => $request_no, "item_fg_id" => $item_fg_id], $post);
                        echo $send;
                    }else{
                        $send = $this->crud->create('transaction_fg', $post);
                        echo $send;
                    }
                }
            // } else {
    ...
    public function delete()
    {
        ...
        $send = $this->crud->delete('transaction_fg', ["id" => $data['id']]);
        echo $send;
    }
    ...
    public function print($option = "")
    {
        if ($option == "excel") {
            ...
            header("Content-Disposition: attachment; filename=transaction_fg_$format.xls");
        }
        // $filter_period = $this->input->get('filter_period');
        // $filter_workorder   = $this->input->get('filter_workorder');
        $filter_request_no = $this->input->get('filter_request_no');
