...
class Purchase_requests extends CI_Controller
{
    ...
    public function readCategoryno()
    {
        ...
        $records = $this->crud->query("SELECT d.id, d.number, d.name
            ...
            JOIN supplier_items b on a.item_id = b.item_id
            JOIN items c on b.item_id = c.id
    ...
    public function datatables()
    {
        ...
        $id = $_POST['id'];
        if ($id === "0") {
            $this->db->select('request_no, request_date, expected_date, request_name, sum(a.qty) as qty, a.status, c.id as item_family_id');
            $this->db->from('purchase_requests a');
            $this->db->join('items b', 'a.item_id = b.id');
            $this->db->join('item_familys c', 'b.item_family_id = c.id');
            $this->db->where('a.deleted', 0);
            ...
            $this->db->join('items b', 'a.item_id = b.id');
            $this->db->join('item_familys c', 'b.item_family_id = c.id');
            $this->db->join('purchase_orders d', 'a.request_no = d.request_no and a.item_id = d.item_id', 'left');
    ...
    public function datatable_updates(){
        ...
        $records = $this->crud->query("SELECT c.number as item_number, c.name as item_name, c.id as item_id, a.qty, a.remarks
            FROM purchase_requests a
    ...
    public function create()
    {
        if ($this->input->post()) {
            if ($this->form_validation->run() == TRUE) {
                ...
                $purchase_request_item = $this->crud->read('purchase_requests', [], ["request_no" => $post['request_no'], "item_id" => $post['item_id']]);
                if (@$purchase_request_item->id != "") {
                    $send = $this->crud->update('purchase_requests', ["request_no" => $post['request_no'], "item_id" => $post['item_id']], $post);
                } else {
    ...
    public function upload()
    {
        ...
        $item_family = $this->crud->read('item_familys', [], ["number" => $category]);
        if (!empty($item_family)) {
            ...
            for ($i = 4; $i <= $total_row; $i++) {
                $datas[] = array(
                    ...
                    'request_date' => date("Y-m-d", $data->val($i, 2)),
    ...
    public function uploadcreate()
    {
        if ($this->input->post()) {
            ...
            $purchase_requests = $this->crud->read('purchase_requests', [], ["request_no" => $data['request_no'], "item_id" => $item->id]);
            if (empty($item->id)) {
                ...
                $send   = $this->crud->create('purchase_requests', array_merge(["item_id" => $item->id, "supplier_id" => $supplier->id]));
                echo $send;
            }
        }
    }
    ...
    public function print_request($request_no)
    {
        ...
        $no = 1;
        for ($i = 0; $i < $page; $i++) {
            $this->db->select('a.*, b.number as item_id, b.description as item_name, c.name as uom');
            $this->db->from('purchase_requests a');
            $this->db->join('items b', 'a.item_id = b.id');
    ...
    public function print($option = "")
    {
        ...
        $this->db->select('a.*, b.number as item_id, b.name as item_name, c.name as item_family_name, e.po_no, f.name as uom');
        $this->db->from('purchase_requests a');
        $this->db->join('items b', 'a.item_id = b.id');
        $this->db->join('item_familys c', 'b.item_family_id = c.id');
        $this->db->join('purchase_orders e', 'a.request_no = e.request_no and a.item_id = e.item_id', 'left');
