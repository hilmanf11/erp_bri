...

{
...

    {
        ...
        if($filter_status === "2"){
            $this->db->having('(qty_po - qty_receipt) < ', 0);
        ...
        foreach ($records as $data) {
            ...
            if (($data['qty_po'] - $data['qty_receipt']) > 0) {
                $status = "<b style='color:orange;'>OPEN</b>";
            } else if (($data['qty_po'] - $data['qty_receipt']) < 0){
                $status = "<b style='color:red;'>OVER</b>";
            } else {
                $status = "<b style='color:green;'>CLOSE</b>";
            }
            ...
            $html .= '  <tr>
                            ...
                            <td style="text-align:right">' . number_format($data['qty_po'], 0) . '</td>

                            <td style="text-align:right">' . number_format($data['qty_receipt'], 0) . '</td>

                            <td style="text-align:right">' . number_format($data['qty_po'] - $data['qty_receipt'], 0) . '</td>
                ...
                if ($details) {
                    ...
                    foreach ($details as $detail) {
                        ...
                        $html .= '  <tr>
                                        ...
                                        <td style="text-align:right">' . number_format($detail['qty_receipt'], 0) . '</td>

                                        <td style="text-align:right">' . number_format($detail['qty'] - $detail['qty_receipt'], 0)  . '</td>
