...
class Purchase_order_others extends CI_Controller
{
    ...
    public function datatable_updates(){
        ...
        $records = $this->crud->query("SELECT b.number as item_number, b.name as item_name, a.item_id, d.mpq, d.moq, a.qty, c.currency, d.price, a.total as amount, a.delivery_date, a.remarks
            FROM purchase_order_others a
            JOIN items b on a.item_id = b.id
            JOIN suppliers c on a.supplier_id = c.id
            JOIN supplier_items d on a.supplier_id = d.supplier_id and a.item_id = d.item_id
            JOIN uom e on b.uom_id = e.id
    ...
    public function create()
    {
        if ($this->input->post()) {
            if ($this->form_validation->run() == TRUE) {
                ...
                $purchase_order_others = $this->crud->read('purchase_order_others', [], ["po_no" => $post['po_no'], "item_id" => $post['item_id']]);
                if (@$purchase_order_others->id != "") {
                    $send = $this->crud->update('purchase_order_others', ["po_no" => $post['po_no'], "item_id" => $post['item_id']], array_merge($post, array("revision" => (@$purchase_order_others->revision + 1))));
                } else {
