...
class Bom_compound extends CI_Controller
{
    ...
    public function index()
    {
        if (empty($this->session->username)) {
            ...
            $this->load->view('master/bom_compound');
        } else {
    ...
    {
        @unlink('failed/bom_compound.txt');
    }
    ...
    public function uploadcreateFailed()
    {
        if ($this->input->post()) {
            ...
            $textFailed = fopen('failed/bom_compound.txt', 'a');
    ...
    public function uploadCreate()
    {
        if ($this->input->post()) {
            ...
            } elseif ($item_fg->item_family_number != "CD") {
                echo json_encode(array(
                    "title"   => "Invalid Product",
                    "message" => "Product ID " . $data['item_fg_id'] . " is not registered as Compound Item",
                    "theme"   => "error"
                ));
    ...
    public function print($option = "")
    {
        ...
        <center>
            ...
            <div style="float: centet; font-size: 16px; text-align: center;">
                <h3>MASTER BILL OF MATERIAL COMPOUND</h3>
            </div>
        </center>
    ...
    public function export_excel()
    {
        ...
        $filename = 'bom_compound_' . date('Ymd') . '.xlsx';
    ...
    public function exportTemplate() {
        ...
        $templateSheet->setCellValue('A1', 'TEMPLATE UPLOAD BOM COMPOUND');
        ...
        header('Content-Type: application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
        header('Content-Disposition: attachment; filename="tmp_bom_compound.xls"');
        header('Cache-Control: max-age=0');
