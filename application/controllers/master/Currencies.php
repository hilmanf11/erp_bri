...
class Currencies extends CI_Controller
{
    ...
    public function index()
    {
        if (empty($this->session->username)) {
            ...
            $this->load->view('master/currencies');
        } else {
    ...
    public function create()
    {
        if ($this->input->post()) {
            if ($this->form_validation->run() == TRUE) {
                ...
                $send   = $this->crud->create('currencies', $post);
                echo $send;
            } else {
    ...
    public function update()
    {
        if ($this->input->post()) {
            ...
            $post = $this->input->post();
            $send = $this->crud->update('currencies', ["id" => $id], $post);
            echo $send;
        } else {
            show_error("Cannot Process your request");
        }
    ...
    public function delete()
    {
        ...
        $send = $this->crud->delete('currencies', $data);
        echo $send;
    }
    //PRINT & EXCEL DATA
    public function print($option = "")
    {
        if ($option == "excel") {
            ...
            header("Content-Disposition: attachment; filename=currencies_$format.xls");
        }
        //Config
        $this->db->select('*');
        $this->db->from('config');
