...
class Convertions extends CI_Controller
{
    public function __construct()
    {
        ...
        //VALIDASI FORM
        // $this->form_validation->set_rules('name', 'Name', 'required|min_length[1]|max_length[50]|is_unique[convertions.name]');
    }
    //HALAMAN UTAMA
    ...
    public function reads()
    {
        ...
        $send = $this->crud->reads('convertions', ["item_rm_id" => $post]);
    ...
    public function datatables()
    {
        if ($this->input->post()) {
            ...
            $result = array();
            //Select Query
            $this->db->select('a.*, b.name as item_rm_name, b.number as item_rm_number');
    ...
    public function create()
    {
        if ($this->input->post()) {
            ...
            $send   = $this->crud->create('convertions', $data);
            echo $send;
        } else {
    ...
    public function upload()
    {
        ...
        for ($i = 3; $i <= $total_row; $i++) {
            $datas[] = array(
                ...
                'uom_po' => $data->val($i, 3),
                'uom_soft' => $data->val($i, 4),
                'convertion' => $data->val($i, 5)
            );
        }
        ...
        if ($this->input->post()) {
            ...
            if (empty($item_rm->number)) {
                ...
                $dataFinal = array(
                    ...
                    "uom_po" => $data['uom_po'],
                    "uom_soft" => $data['uom_soft'],
                    "convertion" => $data['convertion'],
                );
                $send   = $this->crud->create('convertions', $dataFinal);
                echo $send;
            }
        }
    }
