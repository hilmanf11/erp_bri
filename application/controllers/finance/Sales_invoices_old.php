...
class Sales_invoices extends CI_Controller
{
        ...
        if ($this->input->post()) {
            ...
            foreach ($sales_invoices as $sales_invoice) {
                if (!empty($sales_invoice['id'])) {
                    $sales_invoice['updated_by'] = $this->session->userdata('username');
                    $sales_invoice['updated_date'] = date('Y-m-d H:i:s');
                    $send = $this->db->update('sales_invoices', $sales_invoice, ["id" => $sales_invoice['id']]);
                } else {
                    $sales_invoice['created_by'] = $this->session->userdata('username');
                    $sales_invoice['created_date'] = date('Y-m-d H:i:s');
                    $send = $this->crud->create('sales_invoices', $sales_invoice);
            ...
            foreach ($sales_invoice_journals as $sales_invoice_journal) {
                if (@$sales_invoice_journal['id'] != "") {
                    ...
                    $send = $this->crud->create('sales_invoice_journals', $sales_invoice_journal);
                }
            }
    ...
    public function update()
    {
        if ($this->input->post()) {
            $post = $this->input->post();
            $post['updated_by'] = $this->session->userdata('username');
