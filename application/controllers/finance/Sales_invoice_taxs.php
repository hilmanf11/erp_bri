...
class Sales_invoice_taxs extends CI_Controller
{
    ...
    public function index()
    {
        if (empty($this->session->username)) {
            ...
            $this->load->view('finance/sales_invoice_taxs');
        } else {
    ...
    public function reads($number)
    {
        ...
        $this->db->from('sales_invoice_taxs a');
    ...
    {
        $data = $this->crud->query("SELECT DISTINCT `number` FROM sales_invoice_taxs WHERE `status` = '0' ORDER BY `number` ASC");
        echo json_encode($data);
    }

    public function readDeliveryNote()
    {
        $data = $this->crud->query("SELECT DISTINCT `dn_number` FROM sales_invoice_taxs WHERE `status` = '0' ORDER BY `dn_number` ASC");
        echo json_encode($data);
    }
    ...
    public function number($trans_date)
    {
        ...
        $sqlGetID   = $this->db->query("SELECT max(`number`) as kode FROM sales_invoice_taxs WHERE `number` like '%$datenow%'");
        $rowID      = $sqlGetID->row();
        $kode       = $rowID->kode;
        if ($kode == NULL) {
    ...
    public function create()
    {
        if ($this->input->post()) {
            if ($this->form_validation->run() == TRUE) {
                ...
                $sales_invoice_taxs = $this->crud->read('sales_invoice_taxs', [], ["dn_number" => $post['dn_number'], "customer_po" => $post['customer_po'], "item_id" => $post['item_id']]);
                if (@$sales_invoice_taxs->id != "") {
                    $send = $this->crud->update('sales_invoice_taxs', ["dn_number" => $post['dn_number'], "customer_po" => $post['customer_po'], "item_id" => $post['item_id']], $post);
                    echo $send;
                } else {
                    $send = $this->crud->create('sales_invoice_taxs', $post);
    ...
    public function update()
    {
        if ($this->input->post()) {
            $post = $this->input->post();
            $send = $this->crud->update('sales_invoice_taxs', ["number" => $post['number']], $post);
            echo $send;
        } else {
            show_error("Cannot Process your request");
        }
    ...
    public function delete()
    {
        ...
        $send = $this->crud->delete('sales_invoice_taxs', $data);
    ...
    public function print($option = "")
    {
        if ($option == "excel") {
            ...
            header("Content-Disposition: attachment; filename=sales_invoice_taxs_$format.xls");
        }
