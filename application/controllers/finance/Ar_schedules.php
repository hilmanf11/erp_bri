...
class Ar_schedules extends CI_Controller
{
    ...
    public function print($option = "")
    {
                    ...
                    <table style="width: 100%;">
                        <tr>
                            ...
                            <td style="font-size: 14px; text-align: left; margin:2px;">
                                <b>' . $config->name . '</b><br>
                                <small>REPORT ACCOUNT RECEIVABLE AGING</small><br>
        ...
        foreach ($customers as $customer) {
            ...
            $pi_current = $this->crud->query("SELECT SUM(total_grand) as total FROM sales_invoices WHERE due_date between '$filter_from' and '$filter_to' and customer_id = '$customer_id' and `status` = '0'");
            ...
            if ($pi_current[0]->total != "") {
                ...
                $si_due = $this->crud->query("SELECT sum(total_grand) as total, datediff('$filter_to', due_date) as difference FROM sales_invoices WHERE customer_id = '$customer_id' and `status` = '0' GROUP BY due_date");

                if (@$si_due[0]->difference >= 1 && @$si_due[0]->difference <= 30) {
                    $one = $si_due[0]->total;
                } elseif (@$si_due[0]->difference >= 31 && @$si_due[0]->difference <= 60) {
                    $two = $si_due[0]->total;
                } elseif (@$si_due[0]->difference >= 61 && @$si_due[0]->difference <= 90) {
                    $three = $si_due[0]->total;
                } elseif (@$si_due[0]->difference > 90) {
                    $four = $si_due[0]->total;
                }
            }
