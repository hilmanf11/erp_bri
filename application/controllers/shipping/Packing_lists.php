...
class Packing_lists extends CI_Controller
{
    ...
    public function index()
    {
        if (empty($this->session->username)) {
            ...
            $this->load->view('shipping/packing_lists');
        } else {
    ...
    {
        $datenow    = "PL" . date("Ymd", strtotime(base64_decode($trans_date)));
    ...
    {
        $post = isset($_POST['q']) ? $_POST['q'] : "";
        $delivery_notes = $this->crud->query("SELECT dn_number as `number`
            FROM sales_invoices
            WHERE `status` = '0' and `dn_number` like '%$post%' 
            GROUP BY `dn_number`
            ORDER BY `dn_number` ASC");
    ...
    public function create()
    {
        if ($this->input->post()) {
            if ($this->form_validation->run() == TRUE) {
                ...
                $packing_lists = $this->crud->read('packing_lists', [], ["dn_number" => $post['dn_number'], "customer_po" => $post['customer_po']]);
                if (@$packing_lists->id != "") {
    ...
    public function print_dn($packing_list)
    {
        ...
        for ($i = 0; $i < $page; $i++) {
            ...
            $this->db->join('uom c', 'b.uom_id = c.id');
            $this->db->join('customers d', 'a.customer_id = d.id');
            $this->db->join('delivery_notes e', 'a.dn_number = e.number');
