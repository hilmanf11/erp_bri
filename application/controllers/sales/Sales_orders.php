...
class Sales_orders extends CI_Controller
{
    ...
    public function readItemFg($customer_id)
    {
        ...
        $send = $this->crud->query("SELECT b.id, b.number, b.name, b.number_customer, a.price, c.currency, b.uom, a.valid_from, a.valid_to, COALESCE(SUM(d.qty_del), 0) as delivery
            FROM customer_items a 
            JOIN item_fg b ON a.item_fg_id = b.id and b.type = 'FG'
            JOIN customers c ON a.customer_id = c.id
