...
class Supply_sheets extends CI_Controller
{
    ...
    public function readPeriod()
    {
        ...
        if($type == "OPEN"){
            ...
            JOIN (SELECT b.workorder, b.item_id, SUM(a.qty) as qty, b.qty_req, (SUM(a.qty) - b.qty_req) as qty_issued 
                        FROM issued_material_details a 
                        JOIN supply_sheets b ON a.request_no = b.request_no and a.item_id = b.component_id
                        GROUP BY a.request_no, a.item_id
                        HAVING (SUM(a.qty) - b.qty_req) < 0) b ON a.item_id = b.item_id and a.workorder = b.workorder
            WHERE a.status = 1
            GROUP BY a.period
    ...
    public function readMPQ($item)
    {
        ...
        $send = $this->crud->query("SELECT b.name, a.mpq FROM supplier_items a JOIN suppliers b ON a.supplier_id = b.id WHERE a.item_id = '$item_id'");
        echo json_encode($send);
    }
    ...
    public function readWp()
    {
        ...
        if($type == "OPEN"){
            ...
            $send = $this->crud->query("SELECT a.wp, b.qty_issued
            FROM production_schedules a 
            LEFT JOIN (SELECT b.workorder, b.item_id, SUM(a.qty) as qty, b.qty_req, (SUM(a.qty) - b.qty_req) as qty_issued 
                        FROM issued_material_details a 
                        JOIN supply_sheets b ON a.request_no = b.request_no and a.item_id = b.component_id
                        GROUP BY a.request_no, a.item_id
                        HAVING (SUM(a.qty) - b.qty_req) < 0) b ON a.item_id = b.item_id and a.workorder = b.workorder
            WHERE a.status = 1 and a.period = '$period' and b.qty_issued is null
            GROUP BY a.wp
            ORDER BY a.wp DESC");
        }
    ...
    public function datatables()
    {
        ...
        if ($id === "0") {
            ...
            $this->db->join('production_schedules e', 'a.item_id = e.item_id and a.workorder = e.workorder');
            $this->db->join('bom f', 'a.item_id = f.item_id and a.component_id = f.component_id');
            $this->db->join("(SELECT a.request_no, a.item_id, SUM(a.qty) as qty, b.qty_req, (SUM(a.qty) - b.qty_req) as qty_issued2 
            FROM issued_material_details a 
            JOIN supply_sheets b ON a.request_no = b.request_no and a.item_id = b.component_id
            GROUP BY a.request_no, a.item_id
            ...
            $this->db->join('production_schedules e', 'a.item_id = e.item_id and a.workorder = e.workorder');
            $this->db->join('bom f', 'a.item_id = f.item_id and a.component_id = f.component_id');
            $this->db->join('convertions g', 'a.component_id = g.item_id', 'left');
            ...
            foreach ($records as $record) {
                ...
                $arr[] = array(
                    ...
                    "item_number" => $record['component_number'],
                    "item_name" => $record['component_name'],
    ...
    public function datatablesTemp()
    {
        ...
        $this->db->select('
            ...
            h.mpq,
            b.operation,
            (CASE WHEN f.uom_id is null THEN e.name ELSE g.name END) as uom,
            (CASE WHEN f.uom_id is null THEN b.qpa ELSE (b.qpa * f.qty) END) as qpa,
            a.qty,
            CASE WHEN d.qty_bal is not null THEN d.qty_bal WHEN f.uom_id is null THEN round(a.qty * b.qpa) ELSE round(a.qty * (b.qpa * f.qty)) END as qty_req,
            CASE WHEN d.qty_bal is not null THEN d.qty_bal WHEN f.uom_id is null THEN round(a.qty * b.qpa) ELSE round(a.qty * (b.qpa * f.qty)) END as qty_act');
        $this->db->from('production_schedules a');
        $this->db->join('bom b', 'a.item_id = b.item_id');
        $this->db->join('items c', 'b.component_id = c.id');
        $this->db->join('supply_sheets d', 'a.workorder = d.workorder and c.id = d.component_id', 'left');
        $this->db->join('uom e', 'c.uom_id = e.id');
        $this->db->join('convertions f', 'b.component_id = f.item_id', 'left');
        $this->db->join('uom g', 'f.uom_id = g.id', 'left');
        $this->db->join('supplier_items h', 'c.id = h.item_id');
        $this->db->where('a.deleted', 0);
        $this->db->where('a.status', 0);
    ...
    public function create()
    {
        if ($this->input->post()) {
            if ($this->form_validation->run() == TRUE) {
                ...
                $wip_balances = $this->crud->read("wip_balances", [], ["item_id" => $component_id], "", "id", "desc");
                $supplier_items = $this->crud->read("supplier_items", [], ["item_id" => $component_id]);
                $supply_sheets = $this->crud->reads("supply_sheets", [], ["request_no" => $post['request_no'], "item_id" => $post['item_id'], "component_id" => $post['component_id']]);
                $dateNow = date("Y-m-d");
                ...
                if (count($supply_sheets) > 0) {
                    ...
                    if ($post['qty_req'] == 0) {
                        echo json_encode(array("title" => "Qty 0", "message" => $post['component_number'] . " Qty 0", "theme" => "error"));
                    } else {
    ...
    public function delete()
    {
        ...
        $update = $this->crud->update('production_schedules', ["workorder" => $data['workorder'], "item_id" => $data['item_id']], ["status" => 0]);
        $delete = $this->crud->delete('wip_balances', ["request_no" => $data['request_no'], "item_id" => $data['component_id']]);
        echo $send;
    }
    ...
    public function print_kanban($request_no, $operation)
    {
        ...
        $this->db->join('items c', 'a.component_id = c.id');
        $this->db->join('uom d', 'c.uom_id = d.id');
        $this->db->join('bom e', 'a.item_id = e.item_id and a.component_id = e.component_id');
        $this->db->join('warehouse_location_items f', "a.component_id = f.item_id and type = 'RM'", 'left');
        $this->db->join('supplier_items g', 'a.component_id = g.item_id');
        $this->db->where('a.deleted', 0);
        $this->db->where('a.status', 0);
        ...
        $subtotal = 0;
        for ($i = 0; $i < $page; $i++) {
            $this->db->select('a.*, (CASE WHEN a.mpq > 0 THEN a.mpq ELSE g.mpq END) as mpq, c.number as component_number, c.name as component_name, e.qpa, f.location, d.name as uom');
            $this->db->from('supply_sheets a');
            $this->db->join('items c', 'a.component_id = c.id');
            $this->db->join('uom d', 'c.uom_id = d.id');
            $this->db->join('bom e', 'a.item_id = e.item_id and a.component_id = e.component_id');
            ...
            foreach ($records as $record) {
                $wip_balances = $this->crud->read("wip_balances", [], ["item_id" => $record['component_id'], "request_no" => $record['request_no']], "", "id", "desc");
                $html .= '  <tr>
                                <td style="text-align:center">' . $no . '</td>
                                <td>' . $record['component_number'] . '</td>
    ...
    public function print($option = "")
    {
        ...
        $this->db->select('a.*, e.period, e.wp, b.number as item_number, c.number as component_number, c.name as component_name, d.name as uom, g.qty_issued2');
        $this->db->from('supply_sheets a');
        $this->db->join('items b', 'a.item_id = b.id');
        $this->db->join('items c', 'a.component_id = c.id');
        $this->db->join('uom d', 'c.uom_id = d.id');
        ...
        foreach ($records as $data) {
            ...
            $html .= '<tr>
                        ...
                        <td>' . number_format($data['qty_act']) . '</td>
                        <td>' . $data['uom'] . '</td>
                        <td>' . $supply_type . '</td>
                    </tr>';
            $no++;
