...
class Os_mpp extends CI_Controller
{
    public function __construct()
    {
        ...
        //VALIDASI FORM
        // $this->form_validation->set_rules('item_fg_id', 'Product No.', 'required|min_length[1]|max_length[50]|is_unique[os_mpp.item_fg_id]');
        $this->form_validation->set_rules('customer_id', 'Customer', 'required|min_length[1]|max_length[50]|is_unique[os_mpp.customer_id]');
    }
    ...
    public function index()
    {
        if (empty($this->session->username)) {
            ...
            $this->load->view('planning/os_mpp');
        } else {
    ...
    public function reads()
    {
        ...
        $send = $this->crud->reads('os_mpp', ["item_fg_id" => $post]);
    ...
    public function create()
    {
        if ($this->input->post()) {
            if ($this->form_validation->run() == TRUE) {
                ...
                $send   = $this->crud->create('os_mpp', $post);
                echo $send;
            } else {
    ...
    public function update()
    {
        if ($this->input->post()) {
            ...
            $post = $this->input->post();
            $send = $this->crud->update('os_mpp', ["id" => $id], $post);
            echo $send;
        } else {
            show_error("Cannot Process your request");
        }
    ...
    public function print($option = "")
    {
        if ($option == "excel") {
            ...
            header("Content-Disposition: attachment; filename=os_mpp_$format.xls");
        }
        ...
        if ($filter_revision == "" && $filter_customer_id == "" && $filter_item_fg_id == "") {
            $html = '<html><head><title>Print Data</title></head><style>body {font-family: Arial, Helvetica, sans-serif;}#os_mpp {border-collapse: collapse;width: 100%;font-size: 12px;}#os_mpp td, #os_mpp th {border: 1px solid #ddd;padding: 2px;}#os_mpp tr:nth-child(even){background-color: #f2f2f2;}#os_mpp tr:hover {background-color: #ddd;}#os_mpp th {padding-top: 2px;padding-bottom: 2px;text-align: left;color: black;}</style><body>
            <center>
