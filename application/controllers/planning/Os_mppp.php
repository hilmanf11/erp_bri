...
class Os_mppp extends CI_Controller
{
    ...
    public function index()
    {
        if (empty($this->session->username)) {
            ...
            $this->load->view('planning/os_mppp');
        } else {
    ...
    public function reads()
    {
        if ($this->input->get()) {
            ...
            $processSub = $this->crud->reads('os_mpp', [], ["number" => $number]);
    ...
    public function readRevision(){
        ...
        $data = $this->crud->query("SELECT DISTINCT revision FROM os_mpp WHERE p_month = '$month' and p_year='$year' ORDER BY revision ASC");
    ...
    public function datatables()
    {
        ...
        if ($this->input->get()) {
            ...
            $filter_start = date("Y-m-d", strtotime($filter_cutoff . "+1 days"));
            $filter_end = date("Y-m-t", strtotime($filter_cutoff));
            // $qmpp = $this->pg->query("SELECT * FROM wip_trx_mpp WHERE wp_date BETWEEN '$filter_start' and '$filter_end'");
            // $dmpp = $qmpp->result_array();
            $itemq = $this->pg->query("SELECT b.bom_com_item, c.item_name, d.pfm_name, SUM(a.total * b.bom_qty_perassy) as total from
                (select assy_no, SUM(qty) as total from wip_trx_mpp WHERE wp_date between '$filter_start' and '$filter_end' GROUP BY assy_no ORDER BY SUM(qty) asc) a
                join mst_bom b on a.assy_no = b.bom_par_item
                join mst_item c on b.bom_com_item = c.item_id
                join r_prodfam d on c.pfm_id = d.pfm_id
                where d.pfm_id like '%$filter_product_family%' and b.bom_com_item like '%$filter_product_no%'
                group by b.bom_com_item, c.item_name, d.pfm_name
                order by d.pfm_name asc, SUM(a.total * b.bom_qty_perassy) desc
            ");
            $items = $itemq->result_array();
            $arr = array();
            foreach ($items as $item) {
                $item_id = $item['bom_com_item'];
                ...
                if ($os_mpp) {
                    if($os_mpp->status == "EMPTY"){
                        $status = "GENERATE";
                    }else{
                        $status = $os_mpp->status;
                    }
                    $arr[] = array(
                        "p_month" => $filter_month,
                        "p_year" => $filter_year,
                        ...
                        "qty" => round($qty),
                        "actual" => round($os_mpp->qty),
                        "status" => $status,
                        "created_by" => $os_mpp->created_by,
                        "created_date" => $os_mpp->created_date,
                        "updated_by" => $os_mpp->updated_by,
                        "updated_date" => $os_mpp->updated_date,
                    );
                } else {
                    $arr[] = array(
                        "p_month" => $filter_month,
        ...
        if ($this->input->post()) {
            ...
            if($post['product_no'] != ""){
                if (count($os_mpp) > 0) {
                    ...
                    $send   = $this->crud->create('os_mpp', $post, "RMS", "os_mpp");
    ...
    public function delete()
    {
        ...
        $send = $this->crud->delete('os_mpp', $data);
        echo $send;
    }
    ...
    public function uploadcreate()
    {
        if ($this->input->post()) {
            ...
            $postFinal = array(
                ...
                "product_family" => @$os_mpp[0]->product_family,
                "actual" => $post['qty'],
                "status" => $post['status']
            );
            if($post['product_no'] != ""){
    ...
    public function print($option = "")
    {
        ...
        if ($option == "excel") {
            ...
            header("Content-Disposition: attachment; filename=os_wo_$format.xls");
        }
        //Config
        $this->db->select('*');
        $this->db->from('config');
