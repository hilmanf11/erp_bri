...
class Sales_order_deliveries extends CI_Controller
{
        ...
        if ($this->input->post()) {
            ...
            if ($qty_so >= ($total + intval($post['qty']))) {
                if (!empty($sales_order_deliveries->id)) {
                    ...
                    $send = $this->crud->update('sales_order_deliveries', ["id"=>$id,"sales_order_no" => $post['sales_order_no'], "customer_id" => $post["customer_id"], "item_fg_id" => $post['item_fg_id']], $post);
                    echo $send;
                }
                // if (empty($sales_order_deliveries->trans_date)) {
    ...
    public function uploadcreate()
    {
        if ($this->input->post()) {
            ...
            $data['customer_id'] = $custorderno->customer_id;
