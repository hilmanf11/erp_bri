...
class Sales_orders extends CI_Controller
{
    ...
    public function datatables()
    {
        if ($this->input->post()) {
            ...
            $result = array();
            //Select Query
            $this->db->select("a.*, case when a.order_type = 1 then 'Regular' when a.order_type = 2 then 'Additional' else 'Regular' end as so_type, b.name as customer_name");
    ...
    public function create()
{
    ...
    public function print($option = "")
    {
        ...
        $this->db->select("a.*, case when a.order_type = 1 then 'Regular' when a.order_type = 2 then 'Additional' else 'Regular' end as so_type, b.name as customer_name, c.number as item_fg_number, c.name as item_fg_name");
        $this->db->from('sales_orders a');
        $this->db->join('customers b', 'a.customer_id = b.id');
