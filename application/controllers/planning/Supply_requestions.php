...
class Supply_requestions extends CI_Controller
{
        ...
        if ($this->input->post()) {
            if ($this->form_validation->run() == TRUE) {
                ...
                if ($post['qty'] == 0) {
                    ...
                    $supplier_items = $this->crud->read("supplier_items", [], ["item_id" => $item_id]);
                    $items = $this->crud->read("items", [], ["id" => $item_id]);
    ...
    public function print_kanban($request_no)
    {
        ...
        for ($i = 0; $i < $page; $i++) {
                        ...
                        <div style="border: 1px solid #ddd; width:100%;">
                            <div style="padding:10px;">
                                ...
                                <div style="float:left; width:10%; text-align:right;">
                                    <img src="' . base_url('assets/image/qrcode/' . $kanban->request_no . '.png') . '" width="80"/>
                                </div> 
                                <table id="customers">
                                    <tr>
                                        <th>No</th>
            ...
            foreach ($records as $record) {
                $item_id = $record['item_id'];
                $wip_balances = $this->crud->read("wip_balances", [], ["item_id" => $record['item_id'], "request_no" => $record['request_no']], "", "id", "desc");
                $dateNow = date("Y-m-d");
