...
class Forecast_compound_used extends CI_Controller
{
    ...
    public function index()
    {
        if (empty($this->session->username)) {
            ...
            $this->load->view('planning/forecast_compound_used');
        } else {
            redirect('error_access');
        }
    }

    public function readCompoundNo($filter_period_month, $filter_period_year)
    {
    ...
    public function print($option = "")
    {
        if ($option == "excel") {
            ...
            header("Content-Disposition: attachment; filename=forecast_compound_used_$format.xls");
        }
        ...
        $this->db->select('
            ...
            a.item_fg_id,
            SUM(a.month_1 * c.composition) as month_1,
            SUM(a.month_2 * c.composition) as month_2,
            SUM(a.month_3 * c.composition) as month_3,
            SUM(a.month_4 * c.composition) as month_4,
            SUM(a.month_5 * c.composition) as month_5,
            SUM(a.month_6 * c.composition) as month_6,
            SUM(a.month_7 * c.composition) as month_7,
            SUM(a.month_8 * c.composition) as month_8,
            SUM(a.month_9 * c.composition) as month_9,
            SUM(a.month_10 * c.composition) as month_10,
            SUM(a.month_11 * c.composition) as month_11,
            SUM(a.month_12 * c.composition) as month_12
        ');
        $this->db->from('forecasts a');
        $this->db->join('item_fg b', 'a.item_fg_id = b.id');
        $this->db->join('bom c', 'a.item_fg_id = c.item_fg_id', 'left');
        ...
        if ($filter_compound_no == "") {
            $html = '<html><head><title>Print Data</title></head><style>body {font-family: Arial, Helvetica, sans-serif;}#forecast_compound_used {border-collapse: collapse;width: 100%;font-size: 12px;}#forecast_compound_used td, #forecast_compound_used th {border: 1px solid #ddd;padding: 2px;}#forecast_compound_used tr:nth-child(even){background-color: #f2f2f2;}#forecast_compound_used tr:hover {background-color: #ddd;}#forecast_compound_used th {padding-top: 2px;padding-bottom: 2px;text-align: left;color: black;}</style><body>
            <center>
            ...
            <br><br>
            <table id="forecast_compound_used" border="1">
            <tr>
                <th width="20">No</th>
                <th>Compound No</th>
            ...
            echo $html;
        } elseif ($filter_compound_no != "") {
            foreach ($records as $data) {
                $filter_compound_no = $data['compound_no'];
            }
            $html = '<html><head><title>Print Data</title></head><style>body {font-family: Arial, Helvetica, sans-serif;}#forecast_compound_used {border-collapse: collapse;width: 100%;font-size: 12px;}#forecast_compound_used td, #forecast_compound_used th {border: 1px solid #ddd;padding: 2px;}#forecast_compound_used tr:nth-child(even){background-color: #f2f2f2;}#forecast_compound_used tr:hover {background-color: #ddd;}#forecast_compound_used th {padding-top: 2px;padding-bottom: 2px;text-align: left;color: black;}</style><body>
            <center>
                <div style="float: left; font-size: 12px; text-align: left;">
                ...
                <div style="float: left; font-size: 12px; text-align: left;">
                    <table style="width: 100%;">
                        <tr>
                            ...
                            <td style="font-size: 14px; text-align: left; margin:2px;">
                                <small><b>' . $month_name . '</b>  <b>' . $filter_period_year . '</b></small><br>
                                <small><b>' . $filter_compound_no . '</b></small>
                            </td>
                        </tr>
                    </table>
                </div>
