    ...
    public function datatables()
    {
        ...
        if ($id === "0") {
            $this->db->select('
                ...
                SUM(CASE WHEN (COALESCE(i.qty_issued, 0) - a.qty_req) < 0 THEN 1 ELSE 0 END) as open_items
            ');
                ...
                if (count($supply_sheets) > 0) {
                    ...
                    if ($post['qty_req'] == 0) {
                        ...
                        if ($item_rm->supply == 0 && $item_rm->item_family_id == 'P06') {
                            $this->crud->create('supply_chemical', array_merge($post, ["qty_issued" => $issued]));
                        } else {
                            // Jika tidak, simpan ke `supply_sheets`
                            $this->crud->create('supply_sheets', array_merge($post, ["qty_issued" => $issued]));
                        }
                        // if ($post['qty_bal'] == 0) {
