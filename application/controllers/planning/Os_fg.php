...
class Os_fg extends CI_Controller

{
...

    {
        ...
        $this->form_validation->set_rules('item_fg_id', 'Item FG', 'required|min_length[1]|max_length[30]|is_unique[os_fg.item_fg_id]');
...

    {
        ...
        $send = $this->crud->reads('os_fg', ["item_fg_id" => $post]);
...

    {
        ...
        if ($this->input->post()) {
            ...
            if ($this->form_validation->run() == TRUE) {
                ...
                $post['transaction_type'] = 'REFG-002';

                $send   = $this->crud->create('os_fg', $post);
...

    {
        ...
        if ($this->input->post()) {
            ...
            $send = $this->crud->update('os_fg', ["id" => $id], $post);
...

    {
        ...
        $send = $this->crud->delete('os_fg', $data);
...

    {
        ...
        if ($this->input->post()) {
            ...
            $textFailed = fopen('failed/os_fg.txt', 'a');
...

    {
        ...
        if ($this->input->post()) {
            ...
            $data['transaction_type'] = 'REFG-002';



            $os_fg = $this->crud->read('os_fg', [], [
            ...
            if (!empty($os_fg->item_fg_id)) {

                echo json_encode(array("title" => "Duplicated", "message" => "Product ID " . $data['item_fg_id'] . " is Duplicate Data", "theme" => "error"));
                ...
                $send = $this->crud->create('os_fg', $data);
...

    {
        ...
        if ($option == "excel") {
            ...
            header("Content-Disposition: attachment; filename=os_fg_$format.xls");
        ...
        $records = $this->db->get()->result_array();
            ...
            <center>
                ...
                <div style="float: centet; font-size: 16px; text-align: center;">
                    ...
                    <h3>DATA OUTSTANDING FINISH GOOD</h3>
