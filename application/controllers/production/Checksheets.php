...
class Checksheets extends CI_Controller
{
    ...
    public function index()
    {
        if (empty($this->session->username)) {
            ...
            $this->load->view('production/checksheets');
        } else {
    ...
    public function readWorkorder($filter = "")
    {
        ...
        JOIN items d ON a.item_id = d.id 
    ...
    public function datatables()
    {
        if ($this->input->post()) {
            ...
            $result = array();
            //Select Query
            $this->db->select('a.*, c.number as product_no, c.name as product_name, e.name as uom');
    ...
    public function print_label($id)
    {
        ...
        $this->db->select('a.*, d.name as customer_name, c.number as product_no, c.name as product_name, e.name as uom');
        $this->db->from('checksheets a');
        $this->db->join('production_schedules b', 'a.workorder = b.workorder and a.wp = b.wp');
        $this->db->join('items c', 'b.item_id = c.id');
