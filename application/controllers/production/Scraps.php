...
class Scraps extends CI_Controller
{
    ...
    public function index()
    {
        if (empty($this->session->username)) {
            ...
            $this->load->view('production/scraps');
        } else {
    ...
    {
        $send = $this->crud->query("SELECT b.id, b.number, b.name, c.name as uom, SUM(a.qty) as qty 
        FROM scraps a 
    ...
    public function create()
    {
        if ($this->input->post()) {
            if ($this->form_validation->run() == TRUE) {
                ...
                $scraps = $this->crud->reads("scraps", [], ["item_id" => $post['item_id'], "document" => $post['document']]);
                if (count($scraps) > 0) {
    ...
    public function print($option = "")
    {
        ...
        LEFT JOIN (SELECT item_id, COALESCE(SUM(qty), 0) as qty_in FROM scraps WHERE `type` = 'IN' AND document like '%$filter_document%' AND DATE_FORMAT(trans_date, '%Y-%m-%d') between '$filter_from' and '$filter_to' GROUP BY item_id) d ON a.id = d.item_id
        LEFT JOIN (SELECT item_id, COALESCE(SUM(qty), 0) as qty_out FROM scraps WHERE `type` = 'OUT' AND document like '%$filter_document%' AND DATE_FORMAT(trans_date, '%Y-%m-%d') between '$filter_from' and '$filter_to' GROUP BY item_id) e ON a.id = e.item_id
        LEFT JOIN (SELECT item_id, COALESCE(SUM(qty), 0) as begin_in FROM scraps WHERE `type` = 'IN' AND document like '%$filter_document%' AND DATE_FORMAT(trans_date, '%Y-%m-%d') < '$filter_from' GROUP BY item_id) f ON a.id = f.item_id
        LEFT JOIN (SELECT item_id, COALESCE(SUM(qty), 0) as begin_out FROM scraps WHERE `type` = 'OUT' AND document like '%$filter_document%' AND DATE_FORMAT(trans_date, '%Y-%m-%d') < '$filter_from' GROUP BY item_id) g ON a.id = g.item_id
        WHERE b.number != 'FG' and a.item_family_id like '%$filter_family_id%' and a.id like '%$filter_item_id%'
        GROUP BY a.id
        ORDER BY a.number");
