...
class Label_box extends CI_Controller

{
    ...
    public function index()
    {
        if (empty($this->session->username)) {
            ...
            $this->load->view('control/label_box');
        } else {
    ...
    public function readitems()
    {
        ...
        $send = $this->crud->query("
            SELECT item_fg.*, label_packing.qty_packing as qty_packing
            FROM label_packing 
            JOIN item_fg ON item_fg.id = label_packing.item_fg_id 
            WHERE item_fg.number LIKE '%$post%' 
            OR item_fg.number_customer LIKE '%$post%' 
            OR item_fg.name LIKE '%$post%' 
            OR item_fg.id LIKE '%$post%'
        ");
    ...
    public function datatablesTemp()
    {
        ...
        $this->db->select('a.*, b.qty_packing as qty, b.serial_label as serial_label, c.number as item_fg_number, c.name as item_fg_name, a.qty_packing as qty_packing, c.uom, a.compound_lot, a.trans_date');
        $this->db->from('label_packing a');
        $this->db->join('label_packing_detail b', 'b.serial_no = a.serial_no');
    ...
    public function datatables()
    {
        if ($this->input->post()) {
            ...
            //Select Query
            $this->db->select("a.*, b.number as item_number, b.name as item_name, b.uom as uom");
            $this->db->from('item_label_box a');
            $this->db->join('item_fg b', 'a.item_fg_id = b.id');
            $this->db->like('a.item_fg_id', $filter_item_fg);
            $this->db->group_by('a.serial_box');
    ...
    {
        if ($this->input->get('serial_box')) {
            $serial_box = base64_decode($this->input->get('serial_box'));

            $this->db->select("a.*, b.serial_label, c.name as item_name, c.number as item_number, c.uom as uom");
            $this->db->from('item_label_box a');
            $this->db->join('label_packing_detail b', 'a.serial_label = b.serial_label');
            $this->db->join('item_fg c', 'a.item_fg_id = c.id');
            $this->db->where('a.serial_box', $serial_box);
    ...
    public function create()
    {
        if ($this->input->post('data')) {
            ...
            foreach ($data as $row) {
                ...
                $this->db->insert('item_label_box', $insertData);
            }
