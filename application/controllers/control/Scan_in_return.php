...
class Scan_in_return extends CI_Controller
{
    ...
    public function index()
    {
        if (empty($this->session->username)) {
            ...
            $this->load->view('control/scan_in_return');
        } else {
            redirect('error_access');
        }
    }

    public function getScanInReturn()
    {
        $this->db->select('
            a.scan_id,
            a.item_fg_id,
    ...
    public function getChecksheetLabel()
    {
        if ($this->input->post()) {
            ...
            $label = $this->db->get_where('return_visual_checker_label', [
            ...
            $this->db->join("return_visual_checker_label b", "a.scan_id = b.scan_id and a.item_fg_id = b.item_fg_id");
            ...
            $this->db->select("a.item_fg_id, a.serial_label, b.workorder_label, b.compound_lot_no, b.qty");
            $this->db->from('return_visual_checker_label a');
            $this->db->join('return_visual_checker_label_lot_tracking b', 'a.serial_label = b.serial_label', 'left');
    ...
    public function create()
    {
        ...
        try {
            ...
            $label_items = $this->db->query("
                ...
                FROM return_visual_checker_label_lot_tracking
                WHERE serial_label = ?
                FOR UPDATE
            ", [$serial_label])->result();
            ...
            foreach ($details as $d) {
                ...
                $this->crud->create('scan_in_return', $data);
            }

            $this->crud->update('return_visual_checker_label', [
                'serial_label' => $serial_label
            ], [
                'status' => 1
            ]);

            $this->crud->update('return_visual_checker_label_lot_tracking', [
                'serial_label' => $serial_label
            ], [
                'status' => 1
            ]);
    ...
    public function saveSummary()
    {
        ...
        try {
            ...
            foreach ($items as $post) {
                $updateReturn = $this->crud->update('scan_in_return', [
                ...
                if (!$updateReturn) {
                    throw new Exception("Failed update Scan In Return {$post['serial_label']}");
                }
            }
